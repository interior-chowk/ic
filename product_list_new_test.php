...
$query = "SELECT p.*,t.tag,
       ...
       END AS final_unit_price
          ...
          INNER JOIN product_tag AS pt ON p.id = pt.product_id -- pt for product_tag
          INNER JOIN tags AS t ON pt.tag_id = t.id -- t for tags
          ";
...
if (!empty($searchTerm)) {
    ...
    $whereConditions[] = "(p.name LIKE '%$searchTermEscaped%' OR t.tag LIKE '%$searchTermEscaped%')";
}
