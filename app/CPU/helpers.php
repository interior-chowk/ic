...
class Helpers
{
    ...
    public static function technical_specification_data($data, $sub_id)
{
    
    $technical_specification = DB::table('key_specification_values')->where('product_id', $data)->first();

    if($technical_specification){
    $category = DB::table('categories')->where('id', $sub_id)->first();
    $fields = ['specification', 'key_features', 'technical_specification', 'other_details'];

    $result = [];

    foreach ($fields as $field) {
        $specValues = json_decode($technical_specification->$field, true);
        $catValues = explode(',', $category->$field);
        
        if (count($catValues) === count($specValues)) { 
            $result[$field] = array_combine($catValues, $specValues);
        } else {
            $result[$field] = []; // Agar mismatch ho toh empty array return karega
        }
    }
...
public static function sku_product_new($id, $var)
{
    $vars = json_decode($var, true);
    $variations = [];

    if ($vars) {
        foreach ($vars as $v) {
            $results = DB::table('sku_product_new')
                ->where('product_id', $id)
                ->where('variation', $v['type'])
                ->get();

            foreach ($results as $result) {
                $variations[] = $result; // Flattening array
            }
        }
    }
    ...
    {
        $x = ['kg', 'pc', 'gms', 'ltrs', 'Set', 'Pair', 'Sqft', 'Sq.Mtr', 'Box'];
        return $x;
    }
    ...
    {
        //$amount = ($price / 100) * $tax;   100*18/(100+18) = 15.25 nclusive Amount = 100 - 15.25 = 84.75

            $amount_cal = $price / (1 + ($tax/100));
            
         
            
            $tax_amount = $price - $amount_cal;
    ...
    public static function send_push_notif_to_topic_old($data)
    {
        $key = BusinessSetting::where(['type' => 'push_notification_key'])->first()->value;
        ...
        public static function send_push_notif_to_topic_new11($data, $device_token)
        {
            $server_key = BusinessSetting::where(['type' => 'push_notification_key'])->first()->value;
        
            // Firebase Cloud Messaging API URL for v1
            $url = "https://fcm.googleapis.com/fcm/send";
            ...
            $postdata = [
                'to' => $device_token,  // Target device token or topic
                'notification' => [
                    'title' => $data->title,
                    'body' => $data->description,
                    'image' => $image,
                ...
                'data' => [
                    'extra_payload' => 'custom_data',  // Additional custom data
                ],
            ];
       ...
       public static function send_push_notif_to_topic_later($data, $device_token)
        {
            $key = BusinessSetting::where(['type' => 'push_notification_key'])->first()->value;
            $project_id = 'interior-chowk';
            //$url = "https://fcm.googleapis.com/fcm/send";
        ...
        public static function send_push_notif_to_device_new($data, $device_token)
        {
            $project_id = 'interior-chowk'; // Replace with your Firebase project ID
            ...
            $image = asset('storage/notification') . '/' . $data->image; // Update image path as needed
            $postdata = json_encode([
                'message' => [
                    'token' => $device_token, // Use 'token' for sending to specific device
                    'data' => [
                        'title' => $data->title,
                        'body' => $data->description,
                        'image' => $image,
                        'is_read' => '0', // Ensure this is a string
                    ],
                    'notification' => [
                        'title' => $data->title,
                        'body' => $data->description,
            ...
            if (isset($resultData['success']) && $resultData['success'] > 0) {
                return [
                    'success' => true,
                ...
                return [
                    'success' => false,
                    'message' => 'Failed to send notification. Response: ' . $result,
                ];
            }
        }
        
   public static function send_push_notif_to_device_noti($notification)
    {
        ...
        $message = CloudMessage::fromArray([
            'token' => $user->cm_firebase_token, // Specific device token
            'notification' => [
                'title' => $notification->title,
                'body' => $notification->description,
            ],
            'data' => [
                'image' => asset('storage/notification/' . $notification->image),
                'is_read' => '0', // Converted to string
            ],
        ]);
        ...
        /*$project_id = 'interior-chowk'; // Your project ID
        $access_token = self::get_firebase_access_token();
        $url = "https://fcm.googleapis.com/v1/projects/{$project_id}/messages:send";
        ...
        $postdata = json_encode([
            'message' => [
                'token' => $user->cm_firebase_token, // Sending to a specific device token
                'notification' => [
                    'title' => $notification->title,
                    'body' => $notification->description, 
                ],
                'data' => [
                    'image' => (string)asset('storage/notification/' . $notification->image),
                    'is_read' => (string)0, // Convert integer to string
                ],
            ],
        ]);
        }
       ...
       $postdata->send($postdata);*/
    }


        
     public static function send_push_notif_to_topic($data, $device_token)
    {
        $project_id = 'interior-chowk';
        $access_token = self::get_firebase_access_token();
        $url = "https://fcm.googleapis.com/v1/projects/{$project_id}/messages:send";
        ...
        // Update the payload to send to a specific device using the 'token' field
        $postdata = json_encode([
            'message' => [
                'token' => $device_token, // Use 'token' instead of 'topic'
                'data' => [
                    'title' => $data->title,
                    'body' => $data->description,
                    'image' => $image,
    ...
    public static function send_push_notif_to_topic_22_10_old($data, $device_token)
    {
    ...
    {
        $carts = CartManager::get_carts($cart_group_id);
       
        $cart_summery = OrderManager::order_summary_before_place_order($carts, $coupon_discount);
       // dd($cart_summery['order_total']);
       $commission_amount = self::seller_sales_commission(
        $carts[0]->seller_is, 
        $carts[0]->seller_id, 
        $cart_summery['order_total']
    );
       // dd('hello');
        return $commission_amount;
    }
    ...
    private static function get_firebase_access_token()
    {
        
       try {
        // Path to the Firebase service account key file
        $service_account_key_file = storage_path('app/firebase-service-account.json');
        ...
        // Fetch access token
        $access_token_response = $client->fetchAccessTokenWithAssertion();
        
        // Check if any error occurred during token retrieval
        if (isset($access_token_response['error'])) {
            throw new \Exception('Error fetching access token: ' . $access_token_response['error']);
        }
        ...
        \Log::error('Error in fetching Firebase access token: ' . $e->getMessage());
