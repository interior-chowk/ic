...
class OrderManager
{
    ...
    public static function generate_order($data)
    {
        ...
        $getReq = function ($key, $default = null) use ($req) {
            if ($req instanceof \Illuminate\Http\Request) {
                return $req->input($key, $default);
            }
            if (is_array($req) || $req instanceof ArrayAccess) {
        ...
        $coupon_code_from_req = $getReq('coupon_code', session('coupon_code'));
        if ($coupon_code_from_req) {
            $coupon = Coupon::where('code', $coupon_code_from_req)->where('status', 1)->first();
            $coupon_process = $coupon ? self::coupon_process($data, $coupon) : $coupon_process;
        }

        $order_id = 1100 + Order::count() + 1;
        if (Order::find($order_id)) {
            $order_id = Order::orderByDesc('id')->first()->id + 1;
        }

        $address_id = session('shipping_address_id') ?? $getReq('shipping_address_id', null);
        $billing_address_id = session('billing_address_id') ?? $getReq('billing_address_id', null);
        
        $order_note = session('order_note');

        $discount = $getReq('coupon_discount') 
        ...
        $shipping_fees_from_cart = CartShipping::where('cart_group_id', $cart_group_id)->value('shipping_cost') ?? 0;

        $shipping_model = Helpers::get_business_settings('shipping_method');
        $shipping_type = 'order_wise';
        if ($shipping_model === 'inhouse_shipping' || ($seller_data && $seller_data->seller_is === 'admin')) {
        ...
        $wallet_deduct_amount = floatval($getReq('wallet_deduct_amount') ?? session('wallet_deduct_amount', 0));
        if ($wallet_deduct_amount > 0 && $user) {
            $user_data = User::where('id', $user->id)->first();
            if ($user_data) {
                $previousBalance = floatval($user_data->wallet_balance ?? 0);
                $newBalance = max(0, $previousBalance - $wallet_deduct_amount);
                $user_data->wallet_balance = $newBalance;
                $user_data->save();

                try {
                    $wdata = new CustomerWalletHistory;
                    $wdata->customer_id = $user->id;
                    $wdata->transaction_type = 0; // deduction
                    $wdata->transaction_amount = $wallet_deduct_amount;
                    $wdata->transaction_method = "Deduction";
                    $wdata->save();
                } catch (\Exception $ex) {
                    Log::error("CustomerWalletHistory save failed: ".$ex->getMessage());
                }
                ...
                try {
                    ...
                    $wallet_transaction->balance = $newBalance;
                    $wallet_transaction->transaction_type = "deduction";
                    $wallet_transaction->reference = 'Order (pre)';
                    $wallet_transaction->save();
                } catch (\Exception $ex) {
                    Log::error("WalletTransaction save failed: ".$ex->getMessage());
                }
            }
        }
        ...
        if (!empty($getReq('instant_delivery'))) {
            $instant_delivery = 1;
        }

        session()->forget('instant_delivery');
        $shipping_fee_details = $data['shipping_fee'] ?? CartManager::get_shipping_cost($cart_group_id);
        $shipping_fee_applied = ($getReq('shipping_fee', 0) == '0.00') ? 0 : ($data['shipping_fee_value'] ?? $shipping_fees_from_cart ?? 0);

        if (is_array($shipping_fee_details)) {
            $shipping_cost_amt = json_encode($shipping_fee_details);
        } else {
            $shipping_cost_amt = is_string($shipping_fee_details) ? $shipping_fee_details : json_encode($shipping_fee_details);
        }

        $amount_from_req = $getReq('amount', session('checkout_amount'));
        $amount_from_req = is_null($amount_from_req) ? null : floatval($amount_from_req);

        if ($amount_from_req === null && is_object($req) && isset($req->amount)) {
            $amount_from_req = floatval($req->amount);
        }

        $order_amount_calculated = CartManager::cart_grand_total($cart_group_id) - $discount;

        $order_data = [
            'id' => $order_id,
            'invoice_no' => "IC/2025-2026/".$order_id,
            'verification_code' => rand(1000000, 9999999),
            'customer_id' => $user->id ?? null,
            'seller_id' => $seller_data->seller_id ?? null,
            'seller_is' => $seller_data->seller_is ?? null,
            ...
            'order_amount' => $order_amount_calculated,
            ...
            'shipping_cost' => $data['shipping_cost'] ?? $shipping_fees_from_cart ?? 0,
            'amount' => $amount_from_req,
            'shipping_cost_amt' => $shipping_cost_amt,
            'shipping_method_id' => $shipping_method_id,
            'shipping_type' => $shipping_type,
            'cashback_amount' => null,
            'wallet_deduction' => $wallet_deduct_amount,
            'instant_delivery_type' => $instant_delivery,
            'gst_number' => $getReq('gst_no') ?? session('gst_no'),
            'company_name' => $getReq('company_name') ?? session('company_name'),
        ...
        try {
            if ($wallet_deduct_amount > 0 && $user) {
                WalletTransaction::where('user_id', $user->id)
                    ->where('reference', 'Order (pre)')
                    ->orderByDesc('id')
                    ->limit(1)
                    ->update(['reference' => 'Order #' . $order_id]);
            }
        } catch (\Exception $ex) {
            Log::error("WalletTransaction reference update failed: " . $ex->getMessage());
        }
        ...
        foreach (CartManager::get_carts($cart_group_id) as $c) {
            ...
            DB::table('order_details')->insert([
                ...
                'qty' => $c->quantity,
                'price' => $sku->variant_mrp ?? 0,
                'tax' => $sku->tax ?? 0,
                'tax_model' => 'exclude',
                'discount' => $sku->discount ?? 0,
                'discount_type' => $sku->discount_type ?? null,
                'variant' => $sku->variation ?? null,
                'variation' => $variation,
                'delivery_status' => 'pending',
                'shipping_method_id' => $shipping_method_id,
                'payment_status' => $data['payment_method'],
                'created_at' => now(),
                'updated_at' => now()
            ]);

            if ($sku && isset($sku->quantity)) {
                DB::table('sku_product_new')
                    ->where('id', $c->variation)
                    ->where('quantity', '>', 0)
                    ->decrement('quantity', $c->quantity);
            }
        }
        ...
        try {
            $fcm_token = $user->cm_firebase_token ?? null;
            $seller_fcm_token = $seller->cm_firebase_token ?? null;
            ...
            if (!empty($mail_config) && $mail_config['status'] == 0) {
                $mail_config = Helpers::get_business_settings('mail_config_sendgrid');
            }

            if (!empty($mail_config) && $mail_config['status'] == 1) {
            ...
            \Log::error('Order notification error: '.$ex->getMessage());
        }
