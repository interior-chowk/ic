...
class CartManager
{
    ...
    public static function update_to_cart($prod_id , $user_id , $str){

        
            $cart = Cart::where(['product_id' => $prod_id, 'customer_id' => $user_id, 'variant' => $str])->first();
            ...
            if ($str != null) {
                        ...
                        for ($i = 0; $i < $count; $i++) {
                            if (json_decode($product->variation)[$i]->type == $str) {
                                ...
                                $new_sku  = DB::table('sku_product_new')->where('product_id',$prod_id)->where('variation',$str)->first();
                                $price =  $new_sku->variant_mrp;
                                $tax =  $new_sku->tax;
                                $discount = $new_sku->discount;
                                $discount_type = $new_sku->discount_type;
    ...
    public static function get_carts($cart_group_id){
        //dd($request);
        $cart = DB::table('new_cart')->where('cart_group_id', $cart_group_id)->where('is_selected',1)->get();
    ...
    {     
        if ($group_id == null) {
            $cart = DB::table('new_cart')->whereIn('cart_group_id', CartManager::get_cart_group_ids($request))->get();
        } else {
            $cart = DB::table('new_cart')->where('cart_group_id', $group_id)->get();
        }
    ...
    public static function get_cart_group_ids($request = null)
    {
        ...
        if ($user == 'offline') {
           ...
           $cart_ids = DB::table('new_cart')->where(['user_id' => $user->id])->groupBy('cart_group_id')->pluck('cart_group_id')->toArray();
        }
      // dd($cart_ids);
        return $cart_ids;
    ...
    {
        if ($group_id == null) {
            // total shipping cost from cart_shippings for all groups
            return CartShipping::whereIn('cart_group_id', CartManager::get_cart_group_ids())
        ...
        // shipping cost for a specific group
        return CartShipping::where('cart_group_id', $group_id)
            ->whereHas('cart', function ($query) {
                $query->where('product_type', 'physical');
            })
            ->sum('shipping_cost');
    ...
    public static function cart_clean($request = null)
    {
        ...
        DB::table('new_cart')->whereIn('cart_group_id', $cart_ids)->delete();
    ...
    public static function add_to_cart($request, $from_api = false)
    {
      // dd($request->id);
        ...
        ->whereIn('product_id', (array) $request->id) // Ensure array type
        ->whereNotNull('thumbnail_image')
        ->select('product_id', 'thumbnail_image', 'image', 'listed_price')
        ->first();
    
    //dd($sku_products->thumbnail_image);
...
// 17/03/2025
        ...
        $cart['thumbnail'] = $sku_products->thumbnail_image;
