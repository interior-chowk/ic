...
class RazorPayController extends Controller
{
    ...
    private function parseShippingFee($value)
    {
        if (!$value) return 0;
        ...
        foreach ($lines as $line) {
            if (str_contains(strtolower($line), 'free')) {
                return 0;
            }
        }
    ...
    //                 $raw = $shippingPrice['data'][0]['rate'] ?? 0;
    //                 $charge = (strtolower(trim($raw)) === 'free') ? 0 : floatval($raw);
    ...
    //                 'gst_no'        => $request->input('company_name',),
    //                 'shipping_cost' => $this->parseShippingFee($request->input('shipping_fee'))
    //             ]);
    ...
    public function createRazorpayOrder(Request $request, ShipyaariService $shiprocketService)
    {
        ...
        // CART GROUP IDS
        // ===============================
        $cart_group_ids = Cart::where('user_id', $user->id)
            ->where('is_selected', 1)
            ->pluck('cart_group_id')
            ->toArray();

        if (empty($cart_group_ids)) {
            return response()->json(['status' => false, 'message' => 'No cart found'], 400);
        }
        ...
        if ($request->iscod === '1') {
            ...
            try {
                ...
                foreach ($cart_group_ids as $group_id) {
                    ...
                    $this->applyShipping($order, $shiprocketService, true);
                    $shiprocketService->createOrder($order);

                    $finalOrderArray[] = ['order_id' => $order_id];
                }
        ...
        // =====================================================
        // 2️⃣ WALLET FULL PAYMENT (₹0)
        // =====================================================
        if ($request->iscod === '0' && $totalAmount <= 0) {
            ...
            try {
                ...
                foreach ($cart_group_ids as $group_id) {
                    ...
                    $this->applyShipping($order, $shiprocketService, false);
                    $shiprocketService->createOrder($order);
    ...
    public function verifyPayment(Request $request, ShipyaariService $shiprocketService)
    {
        ...
        // All validated — create orders seller-wise
        DB::beginTransaction();
        $finalOrderArray = [];
        try {
            ...
            foreach ($cart_group_ids as $group_id) {
                $data = [
                    ...
                    'status'              => 'created',
                    'request'             => $request,
                    // prefer session values for these (already stored in createRazorpayOrder)
            ...
            // clear relevant sessions (note: wallet_deduct_amount intentionally preserved if you want)
    ...
    private function applyShipping($order, $shiprocketService, $isCod = false)
    {
        $details = $order->details()->with('product:id,weight')->get()->toArray();
        ...
        $total_weight = array_reduce($details, function ($carry, $item) {
            $sku = DB::table('sku_product_new')
                ...
                ->where('variation', $item['variant'])
                ->first();
            return $carry + (floatval($sku->weight ?? 0) * $item['qty']);
        }, 0);
        ...
        if (!empty($shippingPrice['data'])) {
            $raw = $shippingPrice['data'][0]['rate'] ?? 0;
            $order->shipping_cost = strtolower($raw) === 'free' ? 0 : floatval($raw);
            $order->save();
        }
    }
    ...
    //     $image_array = [];
    //     if ($request->hasFile('images_s')) {
    //         foreach ($request->file('images_s') as $image) {
    //             if ($image) {
    ...
    //             'customer_id' => $request->user()->id,
    //             'product_id'  => $request->product_id,
    //             'comment'     => $request->w3review,
    //             'rating'      => $request->rating,
    //             'experience'  => json_encode($request->typeAdd), // ✅ checkbox data
    //             'attachment'  => json_encode($image_array),
    //         ]
    //     );
    ...
    public function review_submit(Request $request)
    {
        // dd($request->typeAdd);
        $validator = Validator::make($request->all(), [
            'product_id_review' => 'required',
            'order_id_review'   => 'required',
            'review_remark'   => 'required',
            'rating'     => 'required',
            'images_s.*' => 'image|mimes:jpg,jpeg,png|max:1024',
        ]);
        ...
        Review::updateOrCreate(
            [
                'customer_id' => $request->user()->id,
                'product_id'  => $request->product_id_review,
                'order_id'    => $request->order_id_review,
            ],
            [
                'comment'    => $request->review_remark,
                'rating'     => $request->rating,
                'reason' => json_encode($request->typeAdd),
                'attachment' => json_encode($image_array),
            ]
        );
    ...
    public function status_return(Request $request)
    {
        ...
        if ($order) {
            $totalAmt = ($order->shipping_cost_amt ?? 0) + ($order->refund_shipping_amt ?? 0);
        }

        $dbStatus = strtolower(trim($return->status));
        $changeBy = strtolower(trim($return->change_by));
        ...
        if ($dbStatus === 'approved (shipping + rto charges will be bear by seller)') {
            $currentStatus = "Request Accepted";
        }

        elseif (
            $dbStatus === 'approved (shipping + rto charges will be bear by ic/customer)'
            && $changeBy === 'seller'
        ) {
        ...
        elseif (
            ...
            if ($totalAmt >= 0 && $totalAmt <= 50) {
                $refundAmount = 25;
            } elseif ($totalAmt > 50 && $totalAmt <= 75) {
                $refundAmount = 40;
            } elseif ($totalAmt > 75 && $totalAmt <= 100) {
                $refundAmount = 55;
            } elseif ($totalAmt > 100 && $totalAmt <= 150) {
                $refundAmount = 75;
            } elseif ($totalAmt > 150 && $totalAmt <= 200) {
                $refundAmount = 110;
            } elseif ($totalAmt > 200 && $totalAmt <= 300) {
                $refundAmount = 150;
            } elseif ($totalAmt > 300 && $totalAmt <= 400) {
                $refundAmount = 200;
            } elseif ($totalAmt > 400) {
                $refundAmount = 250;
            }
            
            $totalrefund = $order->amount - $refundAmount;
              
        }

        elseif ($dbStatus === 'rejected' && $changeBy === 'seller') {
            $currentStatus = "Request Submitted";
        }
        ...
        return response()->json([
            ...
            'current'        => $return,
            'refund_amount' => $totalrefund   // ✅ JS ko pass ho raha
        ]);
    }
    ...
    public function delete_return(Request $request)
    {
        ...
        $deleted = RefundRequest::where('order_id', $orderId)
            ->update(['return_deleted' => 1]);
        if ($deleted) {
