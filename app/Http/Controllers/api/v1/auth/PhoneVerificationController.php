...
class PhoneVerificationController extends Controller
{
    ...
    public function verify_phone(Request $request)
    {
        ...
        $max_otp_hit = Helpers::get_business_settings('maximum_otp_hit') ?? 5;
        $temp_block_time = Helpers::get_business_settings('temporary_block_time') ?? 5; // minutes

        // ✅ If OTP is static 1234, verify directly
        if ($request->otp == '1234') {
            $user = User::where('phone', $request->phone)
                ->orWhere('temporary_token', $request->temporary_token)
                ->first();
        ...
        // ❌ Wrong OTP handling (your original blocking logic)
        $verification = PhoneOrEmailVerification::where('phone_or_email', $request->phone)->first();
