...
class OrderController extends Controller
{
    ...
    // public function place_order(Request $request, ShiprocketService $shiprocketService)
    // {
    ...
    //    $carts = DB::table('new_cart')->where('cart_group_id', $cart_group_ids)->get();
    //  //dd($carts->product_type);
    //     $physical_product = false;
    //     foreach($carts as $cart){
    //        // dd($cart->product_type);
    ...
    //                // dd($c);
    //                $sku = DB::table('sku_product_new')->where('product_id',$c['product_id'])->where('variation',$c['variant'])->first();
                  
    //                 return $prev + ($sku->weight * $c['qty']);    
    //             }, 0);
    ...
    public function place_order(Request $request, ShipyaariService $shiprocketService)
{
    ...
    public function refund_request(Request $request)
    {
        ...
        if($loyalty_point_status == 1)
        {
            ...
            {
                return response()->json(['message'=>translate('you have not sufficient loyalty point to refund this order!!'),'status' => false], 202);
            }
        }*/
        ...
        foreach ($allShiprocketOrders as $shiprocket) {
            if(isset($shiprocket->order) && !empty($shiprocket->order)) {
                if($shiprocket && empty($shiprocket->awb_code)) {
                    ...
                    /*if($status !== false) {
                        $shiprocket->status = $status->data->status;
                        $shiprocket->courier_company_id = $status->tracking_data->courier_company_id;
                        $shiprocket->courier_name = $status->tracking_data->courier_name;
                       ...
                       /*ShiprocketCourier::where('shiprocket_order_id', $shiprocket->shiprocket_order_id)
                                          ->update(['scans' => json_encode($status->tracking_data->shipment_track_activities)]);*/
