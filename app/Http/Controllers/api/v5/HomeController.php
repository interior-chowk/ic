...
class HomeController extends Controller
{
    ...
    public function category(Request $request)
    {
        ...
        $category = $category->map(function ($cat){
            $cat->icon = asset('storage/app/public/category/'.($cat->icon));
            return $cat;
        });
    ...
    public function index(Request $request)
    {
        ...
        foreach ($radii as $radius) {
            ...
            $data['worker'] = $workerQuery->get()->map(function ($worker) {
                // Format URLs
                $worker->image = asset('storage/app/public/service-provider/profile/' . $worker->image);
                $worker->adhaar_front_image = asset('storage/app/public/service-provider/' . ($worker->adhaar_front_image ?? ''));
                $worker->adhaar_back_image = asset('storage/app/public/service-provider/' . ($worker->adhaar_back_image ?? ''));
            ...
            $data['contractor'] = $contractorQuery->get()->map(function ($contractor) {
                // Format URLs
                $contractor->image = asset('storage/app/public/service-provider/profile/' . $contractor->image);
                $contractor->adhaar_front_image = asset('storage/app/public/service-provider/' . ($contractor->adhaar_front_image ?? ''));
                $contractor->adhaar_back_image = asset('storage/app/public/service-provider/' . ($contractor->adhaar_back_image ?? ''));
            ...
            $data['architect'] = $architectQuery->get()->map(function ($architect) {
                // Format URLs
                $architect->image = asset('storage/app/public/service-provider/profile/' . $architect->image);
                $architect->adhaar_front_image = asset('storage/app/public/service-provider/' . ($architect->adhaar_front_image ?? ''));
                $architect->adhaar_back_image = asset('storage/app/public/service-provider/' . ($architect->adhaar_back_image ?? ''));
            ...
            $data['interior_designer'] = $interiorDesignerQuery->get()->map(function ($interior_designer) {
                // Format URLs
                $interior_designer->image = asset('storage/app/public/service-provider/profile/' . $interior_designer->image);
                $interior_designer->adhaar_front_image = asset('storage/app/public/service-provider/' . ($interior_designer->adhaar_front_image ?? ''));
                $interior_designer->adhaar_back_image = asset('storage/app/public/service-provider/' . ($interior_designer->adhaar_back_image ?? ''));
