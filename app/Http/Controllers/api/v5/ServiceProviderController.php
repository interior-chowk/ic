...
class ServiceProviderController extends Controller
{
    ...
    public function all_views(Request $request)
    {
        ...
        $users = $query->orderBy('featured', 'desc')->orderBy('created_at', 'desc')->get();
         ...
         $users = $users->map(function ($users) {
                // Format URLs
                $users->image = asset('storage/app/public/service-provider/profile/' . $users->image);
                $users->adhaar_front_image = asset('storage/app/public/service-provider/' . ($users->adhaar_front_image ?? ''));
                $users->adhaar_back_image = asset('storage/app/public/service-provider/' . ($users->adhaar_back_image ?? ''));
    ...
    public function all_favorite_provider(Request $request)
    {
        ...
        $user->serviceProvider->image = asset('storage/app/public/service-provider/profile/' . $user->serviceProvider->image);
        $user->serviceProvider->adhaar_front_image = asset('storage/app/public/service-provider/' . ($user->serviceProvider->adhaar_front_image ?? ''));
        $user->serviceProvider->adhaar_back_image = asset('storage/app/public/service-provider/' . ($user->serviceProvider->adhaar_back_image ?? ''));
         }
            return $user;
        });
    ...
    public function all_favorite_review(Request $request)
    {
        ...
        $formattedUsers = $users->map(function ($user) {
            // Format URLs
            $user->customer->image = asset('storage/app/public/profile/' . $user->customer->image);
            $user->serviceProvider->image = asset('storage/app/public/service-provider/profile/' . $user->serviceProvider->image);
            $user->serviceProvider->adhaar_front_image = asset('storage/app/public/service-provider/' . ($user->serviceProvider->adhaar_front_image ?? ''));
            $user->serviceProvider->adhaar_back_image = asset('storage/app/public/service-provider/' . ($user->serviceProvider->adhaar_back_image ?? ''));
            return $user;
