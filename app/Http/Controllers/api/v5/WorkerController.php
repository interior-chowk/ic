...
class WorkerController extends Controller
{
    ...
    public function update_profile(Request $request)
    {
      ...
      //  $user->role = 2;  
        ...
        if($request->service_provider_role){
           if($request->service_provider_role == 2)
        {
          $user->role_name = 'Worker';  
          $user->role = 2;  
        }  
         elseif($request->service_provider_role == 3)
        {
        ...
        $user->adhaar_back_image = ImageManager::upload('service-provider/', 'png', $request->file('adhaar_back_image'));
         ...
         $user->banner_image = asset('storage/banner/' . $banner_image);
         }
         //contact $firmDetails
        ...
        $token = $user->createToken('LaravelAuthApp')->accessToken;
                ...
                $data->image = asset('storage/service-provider/profile/' . $data->image);
                $data->adhaar_front_image = asset('storage/service-provider/' . ($data->adhaar_front_image ?? ''));
                $data->adhaar_back_image = asset('storage/service-provider/' . ($data->adhaar_back_image ?? ''));
    ...
    public function update_profile_old(Request $request)
    {
       ...
       if($request->payout_ways){
        ...
        if($request->serviceTypeId){
        $user->serviceTypeId = json_encode($request->serviceTypeId);
        }
