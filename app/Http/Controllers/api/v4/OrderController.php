...
class OrderController extends Controller
{
    ...
    public function check_zip(Request $request, ShipyaariService $shiprocketService)
    {
        
        //dd($request->all()); // testing only
        $data = $shiprocketService->checkForAvailability([
            'pickupPincode' => (int) $request->pickup_postcode,
            'deliveryPincode' => (int) $request->delivery_postcode,
    ...
    public function order_cancel(Request $request, ShiprocketService $shiprocketService)
    {
        $validator = Validator::make($request->all(), [
            'order_id' => 'required'
        ]);
    ...
    public function place_order_by_offline_payment(Request $request, ShiprocketService $shiprocketService)
    {
        $cart_group_ids = CartManager::get_cart_group_ids($request);
        $carts = Cart::whereIn('cart_group_id', $cart_group_ids)->get();
