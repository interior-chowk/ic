...
class CartController extends Controller
{
    ...
    public function addToCart_1(Request $request)
    {
        ...
        $colors = $request->input('colors', null);  // optional color
        ...
        $product = DB::table('sku_product_new')
            ...
            ->where('sku_product_new.product_id', $id)
            ->where('sku_product_new.variation', $variant)
            ->select(
                'sku_product_new.*',
                'products.name',
                'products.user_id',
        ...
        // ✅ Check if same product with same variation & color already exists
        $existing = DB::table('new_cart')
            ->where('user_id', $user->id)
            ->where('product_id', $id)
            ->where('colors', $colors)
            ->where('variation', $product->sku_id)
            ->where('type', $type)
            ->first();
        ...
        if ($existing) {
            ...
            $updated = DB::table('new_cart')
                ...
                if($type == 1){
                    $cart[0] = DB::table('new_cart')
                        ...
                        ->where('new_cart.user_id', $user->id)
                        ->where('new_cart.type', 1)
                        ->where('new_cart.id', $existing->id)
                        ->first();
                    session()->put('product_checkout_directly', $cart);

                    return response()->json(['status' => 1, 'message' => 'Successfully added!', 'is_checkout' => 1], 200);
                }
            ...
            $inserted = DB::table('new_cart')->insertGetId([
                'user_id'       => $user->id,
                'product_id'    => $id,
                'cart_group_id' => $cart_group_id,
                'colors'        => $colors,
                'variation'     => $product->sku_id,
                'quantity'      => $quantity,
                'is_selected'   => 1,
    ...
    public function select(Request $request)
    {
        ...
        $cart = DB::table('new_cart')->where('variation',$id)->update([
            'is_selected'=> $select_id,
        ]);

        

        return response()->json(['message' => 'Cart selection updated.']);
    }
    ...
    public function select_address(Request $request)
    {
        ...
        $select_id = $request->status;
        $user = auth()->user()->id;
        // dd($user);
        DB::table('shipping_addresses')->where('customer_id',$user)->update([
            'is_selected' => 0,
            'updated_at' => now()
        ]);
        $cart = DB::table('shipping_addresses')->where('id',$id)->update([
            'is_selected'=> $select_id,
            'updated_at'=>Now()
        ]);        
    ...
    public function updateQuantity_1(Request $request)
    {
        $request->validate([
            'cart_id' => 'required|exists:new_cart,id',
            'quantity' => 'required|integer|min:1'
        ]);

        DB::table('new_cart')->where('id', $request->cart_id)->update([
    ...
    public function removeFromCart_1(Request $request)
    {
        $request->validate([
            'cart_id' => 'required|exists:new_cart,id'
        ]);

        DB::table('new_cart')->where('id', $request->cart_id)->delete();
    ...
    {
        $request->validate([
            'id' => 'required|integer|exists:shipping_addresses,id',
    ...
    public function getCartEdt(Request $request)
    {
        ...
        $groupIds = $request->cart_group_id;
        ...
        foreach ($groupIds as $group_id) {
            ...
            $sellerCart = DB::table('new_cart')
                ->where('cart_group_id', $group_id)
                ->first();

            if (!$sellerCart) {
                $productWiseEdt[$group_id] = null;
                continue;
            }

            $product = Product::select('id', 'user_id', 'free_delivery')
                ->where('id', $sellerCart->product_id)
                ->first();
            ...
            $carts = DB::table('new_cart')->where('cart_group_id', $group_id)->get();
            foreach ($carts as $c) {

                $sku = DB::table('sku_product_new')
                    ->where('product_id', $c->product_id)
                    ->when(!empty($c->variation), function ($q) use ($c) {
                        $q->where('id', $c->variation);
                    })
                    ->first();

                $q = max((int)$c->quantity, 1);

                $w = is_numeric($sku->weight ?? null)
                        ? (float)$sku->weight
                        : (float)($product->weight ?? 0.5);

                $weight += $w * $q;

                $length = max($length, (float)($sku->length ?? 1));
                $width  = max($width,  (float)($sku->breadth ?? 1));
                $height = max($height, (float)($sku->height ?? 1));
            }
            ...
            $shippingPrice = $shipyaariService->checkForAvailability([
            ...
            /* ---------- COST ---------- */
            $cost = is_numeric($shippingPrice['data']->total ?? null)
            ...
            /* ---------- EDT (DATE FORMAT) ---------- */
            $edtDays = is_numeric($shippingPrice['data']->EDT ?? null)
                ? (int)$shippingPrice['data']->EDT
                : null;

            $edtDate = $edtDays
                ? now()->addDays($edtDays)->format('d M Y')
                : null;

            $productWiseEdt[$group_id] = $edtDate;
            ...
            $shipping->estimated_delivery_days = $edtDays;
            $shipping->save();
