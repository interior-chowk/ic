...
class CartController extends Controller
{
    ...
    public function addToCart_1(Request $request)
    {
        ...
        // ✅ Check for existing cart group ID
    ...
    public function updateQuantity_1(Request $request)
    {
        ...
        $cartItem = DB::table('new_cart')
            ->join('sku_product_new', 'new_cart.product_id', '=', 'sku_product_new.product_id')
            ->join('products', 'sku_product_new.product_id', '=', 'products.id')
            ->where('new_cart.id', $request->cart_id) 
            ->select(
                'new_cart.quantity as cart_qty',
        ...
        $totalPrice = $cartItem ? $cartItem->listed_price * $cartItem->cart_qty : 0;
