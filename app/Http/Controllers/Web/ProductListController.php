...
class ProductListController extends Controller
{
    public function products_1(Request $request, $slug)
    {
        ...
        if (!$category) {
            $tags = Tag::where('tag', 'like', "%$slug%")->pluck('id');

            $products = collect();
            $categories = collect();

            if ($tags->isNotEmpty()) {
                $productIds = ProductTag::whereIn('tag_id', $tags)->pluck('product_id');

                $products = Product::whereIn('id', $productIds)->get();

                // get distinct categories for these products
        ...
        if ($category) {
            ...
            if ($tags->isNotEmpty()) {
                ...
                $categoryIds = $products->pluck('sub_category_id')->unique();
                $descendantIds = $categoryIds;
            }
            ...
            if ($descendantIds->isNotEmpty()) {
                ...
                $baseQuery = DB::table('sku_product_new')
                    ...
                    ->leftJoin('key_specification_values as ksv', 'products.id', '=', 'ksv.product_id')
                    ->whereIn('products.sub_category_id', $descendantIds)
                    ->where('products.status', 1)
                    ->whereNotNull('sku_product_new.thumbnail_image')
                    ->select(
                        'sku_product_new.*',
        ...
        // now run base query if we have category IDs
        ...
        if (!empty($filtersInput)) {
            foreach ($filtersInput as $key => $values) {
                ...
                if ($key === 'size') {
                    ...
                    $productsQuery
                        ->whereIn('sku_product_new.sizes', $normalizedSizes)
                        ->distinct();
                } elseif ($key === 'sort') {
        ...
        foreach ($collection as $row) {
            ...
            foreach ($mapping as $catCol => $ksvCol) {
                ...
                foreach ($labels as $i => $label) {
                    ...
                    $slugs = Str::slug($label);
                    $val = $values[$i];
                    $key = $label . '|' . $val;

                    if (!isset($dynamic[$slugs]['_counts'][$key])) {
                        $dynamic[$slugs]['_counts'][$key] = ['label' => $label, 'value' => $val, 'count' => 0];
                    }
                    $dynamic[$slugs]['_counts'][$key]['count']++;
                }
            }
        }
        ...
        foreach ($collection as $product) {
            $specification = $product->specification ?? ''; // fallback to empty string
            $ksv_specification = $product->ksv_specification ?? '[]'; // fallback to empty JSON
            $colorsJson = $product->colors ?? '[]';

            $color_keys = array_map('trim', explode(',', $specification));
            $color_values = json_decode($ksv_specification, true);
            ...
            $codes = json_decode($colorsJson, true);
    ...
    {
        $tag = $request->name;
        $query1 = Product::whereHas('product_tags.items', function ($query) use ($tag) {
            $query->where('tag', 'like', "%$tag%");
        })->pluck('id');

        $baseQuery = DB::table('sku_product_new')
            ->join('products', 'sku_product_new.product_id', '=', 'products.id')
            ->whereIn('products.id', $query1)
            ->leftJoin('key_specification_values as ksv', 'products.id', '=', 'ksv.product_id')
            ->select(
                'sku_product_new.*',
                'products.*',
