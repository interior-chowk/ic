...
class HomeController extends Controller
{
    ...
    public function careerapply(Request $request, $id)
    {
        $request->validate([
            ...
            'resume' => 'required|file', // max in KB
        ]);
        ...
        $career->update([
            'applicants' => $career->applicants + 1
        ]);
    ...
    public function index_1()
    {
        ...
        $Banner_2 = Banner::where('banner_type', 'Banner 2 web')->where('published', 1)->first();
        ...
        $Banner_4 = Banner::where('banner_type', 'Banner 4 web')->where('published', 1)->first();
        $Banner_5 = Banner::where('banner_type', 'Banner 5 web')->where('published', 1)->first();
        $Banner_6 = Banner::where('banner_type', 'Banner 6')->where('published', 1)->first();
        $Banner_7 = Banner::where('banner_type', 'Banner 7 web')->where('published', 1)->first();
        $Banner_8 = Banner::where('banner_type', 'Banner 8 web')->where('published', 1)->first();
        $Banner_9 = Banner::where('banner_type', 'Banner 9 web')->where('published', 1)->first();
    ...
    public function webSendOtp(Request $request)
    {
        ...
        Session::put('referral', $referral); // 👈 store referral
            ...
            /* if(isset($request->Refferal_code)){
             ...
             if($referrer){
              $user = User::where('phone', $request->phone)->first(); 
                     ...
                     $wdata->transaction_amount = BusinessSetting::where('type', 'Signup_refer_point_receiver')->value('value');
                 ...
                 $user->save();
                  ...
                  $ref_wdata = new CustomerWalletHistory;
                     ...
                     $ref_wdata->transaction_amount = BusinessSetting::where('type', 'Signup_refer_point_sender')->value('value');
    ...
    public function web_suggestion(Request $request)
    {
        ...
        if (!empty($sug)) {
            $suggestions = DB::table('tags')
                ...
                ->where('tags.tag', 'like', "%$sug%")
                ->where('products.status', 1) // optional: only active products
            ...
            // Merge results
            // $suggestions = $products->merge($categories);
        }

        return response()->json(['suggestion' => $suggestions]);
    }

    // Route for Enter key search
    public function web_search(Request $request)
    {
        ...
        return response()->json(['slug' => $search]);
    }
