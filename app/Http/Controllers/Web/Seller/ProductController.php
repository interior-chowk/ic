...
class ProductController extends Controller
{
 ...
 //  dd($state_city);
        ...
        return view('seller-views.product.add-new', compact('cat', 'br', 'brand_setting', 'digital_product_setting','category_id', 'sub_category_id', 'sub_sub_category_id','warehouse'));
    }
    ...
    public function state_city(Request $request)
    {
        try {
            ...
            // Generate city options HTML dynamically with city ID
            $options = '';
            foreach ($cities as $city) {
                $options .= '<li><button onclick="addToSelection(this, \'city\', ' . $city->id . ')">' . $city->name . '</button></li>';
            }
    ...
    public function searchCategories(Request $request)
    {
        ...
        $suggestions = Category::where('name', 'like', "%$query%")
         ...
         $cat  = Category::where('name', 'like', "%$query%") ->first();
           ...
           if(!$cat){
            $html = '<div><label>Add Product* </lable><input type="text" class="form-control" name="category" placeholder="Type product name "><label>HSN Code* </lable><input class="form-control" type="text" name="hsn" place placeholder=""><input type="submit" class="btn btn--primary" value="save"></div>';
    ...
    {
       // 'values[]' se data capture hoga
       $speci = $request->input('specifications'); // Get all specification keys
          $allSpecifications = $request->all(); // Get all input values

          foreach ($speci as $index => $value) {
              $specificationName = "specificaton_{$value}"; // Dynamic key
              $specificationValue = $allSpecifications[$specificationName] ?? null; // Fetch value

              // Debug output
              echo "Specification Key: $value<br>";
              echo "Specification Value: $specificationValue<br>";
          }

          // Features handling
          $values = $request->input('features'); // Get all feature keys
          $allFeatures = $request->all(); // Get all input values

          foreach ($values as $index => $value) {
              $featureName = "features_{$value}"; // Dynamic key
              $featureValue = $allFeatures[$featureName] ?? null; // Fetch value

              // Debug output
              echo "Feature Key: $value<br>";
              echo "Feature Value: $featureValue<br>";
          }

          die; // Stop execution for debugging        //'tax_model'             => 'required',
    ...
    public function bulk_import_data(Request $request)
    {
        ...
        $data = [];
        $col_key = ['name', 'category_id', 'sub_category_id', 'sub_sub_category_id', 'brand_id', 'unit', 'min_qty', 'refundable', 'unit_price', 'purchase_price', 'tax', 'discount', 'discount_type', 'current_stock', 'details','hsn_code','return_days','sku','free_delivery','instant_delivery','length','breadth','height','weight','thumbnail','youtube_video_url','tags'];
        $skip = ['youtube_video_url', 'thumbnail'];
        //$skip = ['thumbnail'];
        $countRow = 0;
        foreach ($collections as $collection) {
            ...
            $video_link = " ";
            if($collection['youtube_video_url'])
            {
            ...
            $data = [
                ...
                'unit_price' => $collection['purchase_price'],
                ...
                'thumbnail' => $thumbnail,
                'status' => 0,
                'HSN_code' => $collection['hsn_code'],
                'Return_days' => $collection['return_days'],
                'code' => $collection['sku'],
                'free_delivery' => $collection['free_delivery'],
                'available_instant_delivery' => $collection['instant_delivery'],
