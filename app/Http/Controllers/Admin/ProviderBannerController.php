...
class ProviderBannerController extends Controller
{
    function list(Request $request)
    {
        ...
        if ($request->has('search')) {
            ...
            $banners = ProviderBanner::where(function ($q) use ($key) {
            ...
            $banners = ProviderBanner::orderBy('id', 'desc');
        }
        $banners = $banners->paginate(Helpers::pagination_limit())->appends($query_param);

        return view('admin-views.provider-banner.view', compact('banners', 'search'));
    }
    ...
    {
        if ($request->ajax()) {
            $banner = ProviderBanner::find($request->id);
    ...
    {
        $banner = ProviderBanner::where('id', $id)->first();
        return view('admin-views.provider-banner.edit', compact('banner'));
    }
    ...
    {
        $br = ProviderBanner::find($request->id);
        ImageManager::delete('/banner/' . $br['photo']);
        ProviderBanner::where('id', $request->id)->delete();
