...
class RefundController extends Controller
{
    public function list(Request $request, $status)
    {
       ...
       if($status === 'Filter'){
            $refund_list = $refund_list->latest()->paginate(Helpers::pagination_limit());
                }else{
    ...
    {
        $request->validate([
            'new_order_id' => 'required|integer|unique:orders,order_return_id,' . $id,
            'new_awb_id' => 'required|string|max:255',
            'refund_shipping_amt' => 'required'
        ]);

        $order = Order::findOrFail($id);
        $awb = ShiprocketCourier::where('order_id', $id)->first();
        
        if (!$awb) {
            return back()->with('error', 'AWB record not found for this order.');
        }

        DB::beginTransaction();
        try {
            $order->order_return_id = $request->new_order_id;
            $order->refund_shipping_amt = $request->refund_shipping_amt;
            $awb->awb_code = $request->new_awb_id;

            $order->save();
            $awb->save();

            DB::commit();
            return back()->with('success', 'Order ID and AWB ID updated successfully.');
        } catch (\Exception $e) {
            DB::rollBack();
            return back()->with('error', 'Failed to update order. Error: ' . $e->getMessage());
        }
    }
