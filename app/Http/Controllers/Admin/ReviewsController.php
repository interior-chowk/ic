...
class ReviewsController extends Controller
{
    ...
    function Provider_reviews_list(Request $request)
    {
        $query_param = [];
        if (!empty($request->from) && empty($request->to)) {
            Toastr::warning('Please select to date!');
        ...
        if ($request->has('search')) {
            $key = explode(' ', $request['search']);
            $provider_id = User::where(function ($q) use ($key) {
                foreach ($key as $value) {
                    $q->where('name', 'like', "%{$value}%");
                }
            })->pluck('id')->toArray();
            ...
            $reviews = ProviderReviews::WhereIn('provider_id',  $provider_id)->orWhereIn('customer_id', $customer_id);
            $query_param = ['search' => $request['search']];
        } else {
            $reviews = ProviderReviews::with(['provider', 'customer'])
                ->when($request->product_id != null, function ($q) {
        ...
        $providers = User::whereIn('role',[2,3,4,5])->select('id', 'name')->get();
        $customers = User::whereNotIn('id',[0])->select('id', 'name', 'f_name', 'l_name')->get();
        $customer_id = $request['customer_id'];
        ...
        return view('admin-views.reviews.provider-list', compact('reviews', 'search', 'providers', 'customers', 'from', 'to', 'customer_id', 'provider_id', 'status'));
    }
    ...
    public function provider_review_status(Request $request)
    {
        $review = ProviderReviews::find($request->id);
        $review->status = $request->status;
        $review->save();
        Toastr::success('Review status updated!');
        return back();
