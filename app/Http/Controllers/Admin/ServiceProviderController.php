...
class ServiceProviderController extends Controller
{
    public function provider_list(Request $request)
    {
       ...
       // Apply is_active filter if provided
        if ($isactive !== null) {
                $customers->where('is_active', $isactive);
                $query_param['isactive'] = $isactive;
            }
        ...
        if ($provider_type !== null) {
            ...
            $customers->where(function ($q) use ($provider_type) {
                $q->Where('role', $provider_type); 
                  
            });
            $query_param['provider_type'] = $provider_type;
        }
        
        $customers->whereIn('role', [2,3,4,5])->with(['orders']);
        $customers = $customers->orderBy('featured', 'desc')->orderBy('created_at', 'desc')->paginate(Helpers::pagination_limit())->appends($query_param);
    
       
        return view('admin-views.service-provider.list', compact('customers', 'search', 'provider_type','isactive'));
    }
    ...
    public function featured_status(Request $request)
    {
        ...
        $provider->featured = ($provider->featured == 0 || $provider->featured == null) ? 1 : 0;
        $provider->save();
        $data = $request->status;
    ...
    public function view(Request $request, $id)
    {
        ...
        if (isset($customer)) {
            ...
            return view('admin-views.service-provider.customer-view', compact('customer', 'orders', 'search'));
        }
