...
class OrderController extends Controller
{
    ...
    public function upload_label(Request $request)
    {
        $pdf_label =    ImageManager::upload('pdf_label/', 'pdf', $request->file('pdf_file')); 
        if($pdf_label){
            Order::where('id',$request->order_id)->update(['pdf_label' => $pdf_label ,'pickup_date' => $request->pickup_date ]); 
        Toastr::success('PDF uploaded successfully!');
        }else{
        Toastr::error('PDF not uploaded!');  
        }
    ...
    public function list(Request $request, $status)
    {
        ...
        $provider_type = $request['provider_type'] ?? 'all';
        $from = $request['from'];
        $to = $request['to'];
        ...
        $orders = Order::with(['customer', 'seller.shop', 'shiprocket_courier'])
            ->when($status != 'all', function ($q) use($status){
                $q->where(function ($query) use ($status) {
                    $query->orWhere('order_status', $status);
                });
            })
            ->when($instant != 'all', function ($q) use($instant){
                $q->where(function ($query) use ($instant) {
                    $query->orWhere('instant_delivery_type', $instant);
                });
            })
            ->when($provider_type != 'all', function($q) use($provider_type){
                            $q->whereHas('customer', function ($query) use ($provider_type){
                                $query->where('role', $provider_type);
                            });
                    })
            ...
            ->when($filter,function($q) use($filter){
                $q->when($filter == 'all', function($q){
                    ...
                    ->when($filter == 'instant_delivery', function ($q){
                       
                            $q->where('instant_delivery_type', 1);
            ...
            ->appends(['instant'=>$request['instant'],'search'=>$request['search'],'filter'=>$request['filter'],'from'=>$request['from'],'to'=>$request['to'],'delivery_man_id'=>$request['delivery_man_id']]);
            foreach ($orders as $order){
                $shiproket_status = ShiprocketCourier::where('order_id',$order->id)->value('status');
                 if($shiproket_status)
                 {
                     if($shiproket_status == 'Delivered'){
                      $orderup = Order::find($order->id);    
    ...
    public function details($id, ShiprocketService $shiprocketService)
    {
        ...
        $shipping = ShiprocketCourier::where("order_id", $id)->first();
        $shiprocket_token= BusinessSetting::where('type', 'shiprocket_token')->first();
        $status = '';
        if($shipping)
        {
        if($shipping->awb_code){
         ...
         $orders = Order::with('delivery_man')->where('id',$id)->get();
        ...
        if($order->order_type == 'default_type')
        {
            ...
            return view('admin-views.order.order-details', compact('orders','shipping_address','order', 'linked_orders', 'delivery_men', 'total_delivered', 'company_name', 'company_web_logo', 'physical_product', 'status'));
        }else{
            
            return view('admin-views.pos.order.order-details', compact('orders','order', 'company_name', 'company_web_logo'));
        }
    ...
    public function status(Request $request)
    {
        ...
        OrderManager::stock_update_on_order_status_change($order, $request->order_status);
          ...
          if($request->order_status == 'delivered' && $order->withdraw_requests_status == 0)
        {
        ...
        public function generate_invoice($id)
    {
       ...
       $sku = DB::table('sku_product_new')->where('product_id',$order->details->first()->product_id)->where('variation',$order->details->first()->variant)->first();
       //dd($sku);
        ...
        $data["order"] = $order;
        $warehouse = DB::table('warehouse')->where('seller_id',$seller->id)->first();
        // dd($warehouse);
        $mpdf_view = View::make('admin-views.order.invoice',
            compact('order','sku','warehouse', 'seller', 'company_phone', 'company_name', 'company_email', 'company_web_logo')
        );
        Helpers::gen_mpdf($mpdf_view, 'order_invoice_', $order->id);
    }

    public function service_generate_invoice($id)
    {
        $company_phone =BusinessSetting::where('type', 'company_phone')->first()->value;
        $company_email =BusinessSetting::where('type', 'company_email')->first()->value;
        $company_name =BusinessSetting::where('type', 'company_name')->first()->value;
        ...
        $order = Order::with('seller')->with('shipping')->with('details')->where('id', $id)->first();
        $payments = DB::table('made_payement')->whereRaw("JSON_CONTAINS(invoice_no, '\"$id\"')")->first();
        //dd($payments);
       $sku = DB::table('sku_product_new')->where('product_id',$order->details->first()->product_id)->where('variation',$order->details->first()->variant)->first();
       //dd($sku);
        $seller = Seller::find($order->details->first()->seller_id);
        ...
        $data["client_name"] = $order->customer !=null? $order->customer["f_name"] . ' ' . $order->customer["l_name"]:\App\CPU\translate('customer_not_found');
        $data["order"] = $order;
        $mpdf_view = View::make('admin-views.order.service_invoice',
            compact('order','sku','payments','seller', 'company_phone', 'company_name', 'company_email', 'company_web_logo')
        );
        Helpers::gen_mpdf($mpdf_view, 'order_invoice_', $order->id);
    }
    ...
    //shiprocketStatus
    public function shiprocketStatus(Request $request, ShiprocketService $shiprocketService)
    {
       
        if($request->filled('order_id')) {
            $shipping = ShiprocketCourier::where("order_id", $request->order_id)->first();
            
            if($shipping && empty($shipping->awb_code)) {
              
                $status = $shiprocketService->getOrder($shipping->shiprocket_order_id);
              
                if($status !== false) {
                    $shipping->status = $status->data->status;
                    $shipping->awb_code = $status->data->awb_data->awb ?? '';
                    $shipping->save();
                    ...
                    if($order){
                        if($shipping->status == 'NEW'){
                            ...
                            {
                            $product_id = OrderDetail::where('order_id',$request->order_id)->first();
                            $withdrawRequest = new WithdrawRequest();
                            $withdrawRequest->seller_id = $order->seller_id;
                            $withdrawRequest->product_id = $product_id->product_id;
                            $withdrawRequest->order_id = $order->id;
                ...
                return response()->json(['status' => true, 'data' => $shipping]);
            }else {
                ...
                /*if($status !== false) {
                    ...
                    $shipping->status = $status->tracking_data->shipment_track->current_status;
                    $shipping->courier_company_id = $status->tracking_data->shipment_track->courier_company_id;
                    $shipping->courier_name = $status->tracking_data->shipment_track->courier_name;
                    $shipping->scans = json_encode($status->tracking_data->shipment_track_activities);
                    $shipping->save();
                }*/
                
                    if($status !== false && isset($status->tracking_data->shipment_track[0])) {
                        $shipping->status = $status->tracking_data->shipment_track[0]->current_status;
                        $shipping->courier_company_id = $status->tracking_data->shipment_track[0]->courier_company_id;
                        $shipping->courier_name = $status->tracking_data->shipment_track[0]->courier_name;
                        $shipping->scans = json_encode($status->tracking_data->shipment_track_activities);
                        $shipping->save();
    ...
    //addToShiprocket
    public function addToShiprocket(Request $request, ShiprocketService $shiprocketService)
    {
        ...
        if($request->filled('order_id')) {
            ...
            if(!$shipping) {
                $order = Order::find($request->order_id);
                if($order) {
                    return $shiprocketService->createOrder($order);
                    return response()->json(['status' => true, 'message' => 'Added to shiprocket successfully.']);
                }
            }else {
    ...
    public function cancelShiprocketOrder(Request $request, ShiprocketService $shiprocketService)
    {
        if($request->filled('order_id')) {
            $shipping = ShiprocketCourier::where("order_id", $request->order_id)->first();
            
            if($shipping && $shipping->status !== 'CANCELED') {
                $order = Order::find($request->order_id);
                if($order) {
                    $shiprocketService->cancelOrder($order);
                    return response()->json(['status' => true, 'message' => 'Order canceled on shiprocket successfully.']);
                }
            }else {
                return response()->json(['status' => false, 'message' => 'Already canceled.']);
            }
        }
