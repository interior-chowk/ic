   ...
   public function index(Request $request, $status)
    {
        ...
        $sellers = Seller::with(['orders', 'product', 'shop'])
            ->when($status_active, function ($query) {
                $query->where('status', 'approved');
            })
            ->when($status_inactive, function ($query) {
                $query->where('status', '!=', 'approved');
            })
            ->when($search, function ($query) use ($search) {
                $keywords = explode(' ', $search);
                foreach ($keywords as $value) {
                    if (strpos($value, 'VN') !== false) {
                    ...
                    $query->where(function ($query) use ($value) {
                        if (strcasecmp($value, 'Active') === 0) {
                            ...
                            $query->orWhere('status', 'suspended')
                        ...
                        $query->orWhere('f_name', 'like', "%{$value}%")
                              ...
                              ->orWhereHas('shop', function ($subquery) use ($value) {
                                  $subquery->where('shops.name', 'like', "%{$value}%");
                              });
                    });
                }
            })
    ...
    public function export_excel(Request $request)
    {
        ...
        $sellers = Seller::with(['shop'])->get();
        $data = [];
        
        foreach ($sellers as $seller) {
            $seller->orders_count = $seller->orders->count();
            $seller->products_count = $seller->product->count();
            $seller->name = '';
            if($seller->f_name || $seller->l_name){
            $f_name = $seller->f_name ? $seller->f_name : '';
            $l_name = $seller->l_name ? $seller->l_name : '';
            $seller->name = $f_name.' '.$l_name;
            }
        ...
        foreach ($sellers as $item) {
            ...
            $data[] = [
                'Seller Code'          => $item->id,
                'Company Name (Business name)'          => $item->shop->name,
                'Seller Name  (User name)'              => $item->name,
                'Registration date'           => $Regdate,
                'Phone no. (Registered)'                  => $item->phone,
                'Email id'                => $item->email,
                'Approved/ Not approved'                  => $item->status == 'approved' ? 'Approved' : 'Not approved',
                'Active/ Inactive'                  => $item->status == 'approved' ? 'Active' : 'Inactive',
                'Total products (Active + Non active)'                   => $item->products_count,
                'Total orders'                   => $item->orders_count,
                'Bank Name'         => $item->shop->bank_name,
                'Branch'         => $item->shop->bank_branch,
                'Account no.'         => $item->shop->acc_no,
                'IFSC Code'         => $item->shop->ifsc,
                'GST No'         => $item->shop->gst_no,
                'PAN No'         => $item->shop->pan,
                'Billing address'           => $item->shop->billing_address,
                'Pickup address'         => $item->shop->address,
            ];
        }

        return (new FastExcel($data))->download('seller_list.xlsx');
    }

    public function view(Request $request, $id, $tab = null)
    {
        $seller = Seller::find($id);
        $shop = \App\Model\Shop::where('seller_id', auth('seller')->id())->first();
        if(!isset($seller))
        {
        ...
        return view('admin-views.seller.view', compact('seller','current_date','shop'));
    }

    public function commission_fee_update(Request $request)
    {
       $seller = seller::where('id',$request->seller_id)->first();
       $seller->commission_fee = $request->commission;
       if ($request->commission == 1 || $request->commission == 3) {
        $seller->fee = null; 
    }else{
        $seller->fee = $request->inpercent;
    }
    ...
    if ($request->hasFile('file')) {
        ...
        $seller->file = $filePath; // Database में path save करें
    }
    ...
    public function updateStatus(Request $request)
    {
        ...
        if ($request->status == "approved") {
            ...
            $order->commission_fee = (int)$request->commissions;
             $order->fee = $request->fee;
           $order->file = $request->img;
    ...
    {
        $order = Seller::findOrFail($request->id);
        $order->profile_edit_status = $request->profile_status;
        if ($request->profile_status == 1) {
            Toastr::success('Seller profile has been approved successfully');
        } else if ($request->profile_status == 0) {
            Toastr::info('Seller profile status has been changed successfully');
        } 
        $order->save();
        return back();
    }
    ...
    public function withdrawStatus(Request $request, $id)
    {
        ...
        if ($request->approved == 1) {
            ...
            $withdraw->save();
           $OrderTransaction =   OrderTransaction::where('order_id',$withdraw->order_id)->first();
           $OrderTransaction->status = 'disburse';
