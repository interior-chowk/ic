...
class DashboardController extends Controller
{
    ...
    //     $totalCustomers = OrderDetail::count();
    //     // dd($totalCustomers);
    ...
    public function dashboard(Request $request)
    {
        ...
        // $totalIncomeData = DB::table('order_details')->get();

        $totalIncomeData = DB::table('order_details')
            ->when($request->customer_id && $request->customer_id != 'all', function($q) use ($request) {
                $q->where('order_details.customer_id', $request->customer_id);
            })
            ->when($request->status && $request->status != 'all', function($q) use ($request) {
                $q->where('order_details.status', $request->status);
            })
            ->when($request->from && $request->to, function($q) use ($request) {
                $q->whereBetween('order_details.created_at', [$request->from, $request->to]);
            })
            ->when(!$request->from && !$request->to && $request->date_type == 'this_year', function($q) {
                $q->whereYear('order_details.created_at', now()->year);
            })
            ->when(!$request->from && !$request->to && $request->date_type == 'this_month', function($q) {
                $q->whereMonth('order_details.created_at', now()->month)
                ->whereYear('order_details.created_at', now()->year);
            })
            ->when(!$request->from && !$request->to && $request->date_type == 'last_month', function($q) {
                $q->whereMonth('order_details.created_at', now()->subMonth()->month)
                ->whereYear('order_details.created_at', now()->subMonth()->year);
            })
            ->when($request->search, function($q) use ($request) {
                $q->where(function($q2) use ($request) {
                    $q2->where('order_details.id', 'like', "%{$request->search}%")
        ...
        //     ->join('shiprocket_couriers', 'shiprocket_couriers.order_id', '=', 'order_details.order_id')
        //     ->where('shiprocket_couriers.status', 'DELIVERED')
        //     ->select('order_details.*', 'shiprocket_couriers.delivered_at')
        //     ->get();
        ...
        foreach ($deliveredOrders as $item) {
            ...
            $productDetails = json_decode($item->product_details, true);
            $returnDays = isset($productDetails['Return_days']) ? (int)$productDetails['Return_days'] : 0;

            if ($returnDays <= 0) continue;

            $deliveredDate = Carbon::parse($item->delivered_at);
            $returnExpireDate = $deliveredDate->copy()->addDays($returnDays)->endOfDay();
        ...
        // $totalEarningData = DB::table('order_details')
        //     ->join('sku_product_new', 'sku_product_new.product_id', '=', 'order_details.product_id')
        //     ->join('orders', 'orders.id', '=', 'order_details.order_id')
        //     ->whereRaw(
        //         'sku_product_new.variation COLLATE utf8mb4_unicode_ci = order_details.variant COLLATE utf8mb4_unicode_ci'
        //     )
        //     // ->where('orders.id',1103)
        //     ->select(
        //         'sku_product_new.listed_price',
        //         'orders.shipping_cost',
        //         'orders.discount_amount',
        //         'sku_product_new.commission_fee',
        //         'sku_product_new.listed_percent',
        ...
        $totalEarningData = DB::table('order_details')
            ->join('sku_product_new', 'sku_product_new.product_id', '=', 'order_details.product_id')
            ->join('orders', 'orders.id', '=', 'order_details.order_id')
            ->whereRaw('sku_product_new.variation COLLATE utf8mb4_unicode_ci = order_details.variant COLLATE utf8mb4_unicode_ci')
            ->when($request->customer_id && $request->customer_id != 'all', function($q) use ($request) {
                $q->where('order_details.customer_id', $request->customer_id);
            })
            ->when($request->status && $request->status != 'all', function($q) use ($request) {
        ...
        foreach ($totalEarningData as $row) {

            $listedPrice   = (float) $row->listed_price;
            $shippingCost  = (float) $row->shipping_cost;
            $discount      = (float) $row->discount_amount;
            $commissionFee = (float) $row->commission_fee;
            $listedPercent = (float) $row->listed_percent;

            $discountExtra =
                (($listedPrice + $shippingCost + $discount) * 5) /100;

            $commissionExtra = $commissionFee * $listedPercent / 100;
            $rowTotal = $discountExtra + $commissionExtra;
            $totalEarning += $rowTotal;

            $rows[] = [
                'order_id'        => $row->order_id,
                'discount_extra'  => number_format($discountExtra, 2),
                'commission'      => number_format($commissionExtra, 2),
        ...
        // $deliveredEarningData = DB::table('order_details')
        //     ->join('sku_product_new', 'sku_product_new.product_id', '=', 'order_details.product_id')
        //     ->join('orders', 'orders.id', '=', 'order_details.order_id')
        //     ->join('shiprocket_couriers','shiprocket_couriers.order_id','=','order_details.order_id')
        //     ->whereRaw(
        ...
        $deliveredtotalEarning = 0;
        $rows = [];

        foreach ($deliveredEarningData as $row) {
            ...
            $commissionExtra = $commissionFee * $listedPercent / 100;
            $rowTotal = $discountExtra + $commissionExtra;
            $deliveredtotalEarning += $rowTotal;
        ...
        $deliveredtotalEarning = number_format($deliveredtotalEarning, 2);

        // $ordersCOD = DB::table('order_details')
        //     ->join('sku_product_new', 'sku_product_new.product_id', '=', 'order_details.product_id')
        //     ->whereRaw(
        //         'sku_product_new.variation COLLATE utf8mb4_unicode_ci 
        //         = order_details.variant COLLATE utf8mb4_unicode_ci'
        ...
        // dd($ordersCOD);
        $totalCollectionCOD = 0;

        foreach ($ordersCOD as $cod) {
            $totalCollectionCOD += (
                $cod->listed_price +
                $cod->shipping_cost +
                $cod->discount_amount -
                $cod->wallet_deduction
            );
        }

        // dd($totalCollectionCOD);

        // $totalCollectionCOD = DB::table('orders')->where('payment_method','cash_on_delivery')->sum('amount');
        // $totalCollectionPrepaid = DB::table('orders')->where('payment_method','razor_pay')->sum('amount');
        // $totalCollectionWallet = DB::table('orders')->sum('wallet_deduction');
        ...
        // $cartCount = DB::table('new_cart')->sum('new_cart.quantity');
        ...
        // Total prepaid collection
$totalCollectionPrepaid = DB::table('orders')
    ->when($request->customer_id && $request->customer_id != 'all', function($q) use ($request) {
        $q->where('orders.customer_id', $request->customer_id);
    })
    ->when($request->from && $request->to, function($q) use ($request) {
        $q->whereBetween('orders.created_at', [$request->from, $request->to]);
    })
    ->when(!$request->from && !$request->to && $request->date_type == 'this_year', function($q) {
...
$totalCollectionWallet = DB::table('orders')
    ->when($request->customer_id && $request->customer_id != 'all', function($q) use ($request) {
        $q->where('orders.customer_id', $request->customer_id);
    })
    ->when($request->from && $request->to, function($q) use ($request) {
    ...
    ->sum('wallet_deduction');

// Product count from recently viewed
$productCount = DB::table('recently_view')
    ->join('sku_product_new', 'sku_product_new.product_id', '=', 'recently_view.product_id')
    ->when($request->from && $request->to, function($q) use ($request) {
        $q->whereBetween('recently_view.created_at', [$request->from, $request->to]);
    })
    ->when(!$request->from && !$request->to && $request->date_type == 'this_year', function($q) {
        $q->whereYear('recently_view.created_at', now()->year);
    })
    ->sum('recently_view.counts');
...
$wishlistCount = DB::table('wishlists')
    ->when($request->customer_id && $request->customer_id != 'all', function($q) use ($request) {
        $q->where('wishlists.customer_id', $request->customer_id);
    })
    ->count('wishlists.id');
...
$cartCount = DB::table('new_cart')
    ->when($request->customer_id && $request->customer_id != 'all', function($q) use ($request) {
        $q->where('new_cart.customer_id', $request->customer_id);
    })
    ->sum('new_cart.quantity');

// Total customers (based on order_details)
$totalCustomers = DB::table('order_details')
    ->when($request->customer_id && $request->customer_id != 'all', function($q) use ($request) {
        $q->where('order_details.customer_id', $request->customer_id);
    })
    ->when($request->from && $request->to, function($q) use ($request) {
...
$maxCategorySale = $bestSellingCategories->max('total_qty');
        ...
        return view('admin-views.system.dashboard', compact(
            ...
            'bestSellingCategories',
            'totalEarning',
            'deliveredtotalEarning',
            'deliveredIncome',
            'totalCollectionCOD',
            'totalCollectionWallet',
            'totalCollectionPrepaid',
            'customers',
            'totalIncome',
            'transactions',
            'search',
