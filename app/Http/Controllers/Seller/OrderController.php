...
class OrderController extends Controller
{
    ...
    public function list(Request $request, $status)
    {
        ...
        $seller_pos = \App\Model\BusinessSetting::where('type', 'seller_pos')->value('value');

        // Filters
        $search           = $request->search;
        $filter           = $request->filter ?? 'all';
        $instant          = $request->instant ?? 'all';
        ...
        $orders = Order::with(['customer', 'shipping', 'shippingAddress', 'delivery_man', 'billingAddress'])
            ...
            ->when($filter === 'POS', fn($q) => $q->where('order_type', 'POS'))

            ->when($instant !== 'all', fn($q) => $q->where('instant_delivery_type', $instant))

            ->when($status !== 'all', fn($q) => $q->where('order_status', $status))

            ->when($delivery_status !== 'all', fn($q) =>
                $q->leftJoin('shiprocket_couriers', 'shiprocket_couriers.order_id', '=', 'orders.id')
                ->where('shiprocket_couriers.status', $delivery_status)
            )

            ->select('orders.*')

            ->when($from && $to, fn($q) => $q->whereBetween('orders.created_at', ["$from 00:00:00", "$to 23:59:59"]))

            ->when($search, function($q) use ($search) {
                $q->where(fn($qq) =>
                    $qq->where('orders.id', $search)
                    ->orWhere('orders.order_status', 'like', "%{$search}%")
                    ->orWhere('orders.transaction_ref', 'like', "%{$search}%")
                );
            })

            ->when($delivery_man_id, fn($q) => $q->where('delivery_man_id', $delivery_man_id))
        ...
        foreach ($statuses as $s) {
            $status_counts[$s] = $this->common_query_status_count(
                Order::where(['seller_is'=>'seller','order_status'=>$s,'seller_id'=>$sellerId]), $request
            );
        }

        return view('seller-views.order.list', array_merge($status_counts, [
            'orders'          => $orders,
            'search'          => $search,
            'from'            => $from,
            'to'              => $to,
    ...
    public function service_generate_invoice($id)
    {
       ...
       //dd($sku);
        ...
        $data["order"] = $order;
        $mpdf_view = View::make('seller-views.order.service_invoice',
    ...
    public function details($id)
    {
        ...
        $orders = $orders->first();
        $ship = ShiprocketCourier::where('order_id',$orders->id)->first();
        

        if($order->order_type == 'default_type') {
            return view('seller-views.order.order-details', compact('ship','orders','shipping_address', 'order', 'delivery_men', 'shipping_method', 'total_delivered', 'physical_product'));
        }else{
            return view('seller-views.pos.order.order-details', compact('ship','orders','order', 'physical_product'));
        }
    }
    ...
    public function generate_invoice_customer_gst($order_id)
    {
        $id = $order_id;
        ...
        $data["client_name"] = $order->customer !=null? $order->customer["f_name"] . ' ' . $order->customer["l_name"]:\App\CPU\translate('customer_not_found');
        $data["order"] = $order;
        $mpdf_view = \View::make('seller-views.order.gst-invoice',
            compact('order', 'seller', 'company_phone', 'company_name', 'company_email', 'company_web_logo')
        );
        Helpers::gen_mpdf($mpdf_view, 'order_gst_invoice_', $order->id);
    }
