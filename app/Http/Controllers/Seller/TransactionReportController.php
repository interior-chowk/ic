...
class TransactionReportController extends Controller
{
    ...
    public function sale_register_report(Request $request)
    {
        ...
        $status = $request['status'] ?? 'all';
        $date_type = $request['date_type'] ?? 'this_year';
        $query_param = ['search' => $search, 'status' => $status, 'customer_id' => $customer_id, 'date_type' => $date_type, 'from' => $from, 'to' => $to,'product_id' => $product_id];
       ...
       // $transactions = $transactions->latest('updated_at')->paginate(Helpers::pagination_limit())->appends($query_param);
        ...
        return view('seller-views.transaction.order-sale-register', compact('customers','transactions','product_data','search',
            'order_transaction_chart', 'payment_data','status', 'date_type', 'from', 'to', 'customer_id','products','product_id'));
    }
    ...
    public function order_transaction_export_excel(Request $request)
    {
        ...
        foreach ($transactions as $tran) {
            if($tran['order']) {
                ...
                //$sellerId = $tran->seller_id;
                $sellerId = auth('seller')->id();
                $order = Order::with(['details' => function ($query) use ($sellerId) {
                    $query->where('seller_id', $sellerId);
                     }])->with('customer', 'shipping')
                    ->with('seller')
                    ->where('id', $tran->order_id)->first();
                if($order){
                $tranData[] = array(
                    'Order ID' => $tran->order_id,
                    'Sold by' => $order->seller->shop->name,
                    'Shop Name' => $shop_name,
                    'Shop Address' => $order->seller->shop->billing_address.', '.$order->seller->shop->city.', '.$order->seller->shop->state.', '.$order->seller->shop->country.', '.$order->seller->shop->pincode,
                    'GSTIN' => $order->seller->shop->gst_no,
                    'Pan' => $order->seller->shop->pan,
                    'State/UT code' => $stateId = DB::table('states')->where('name', $order->seller->shop->state)->value('id'),
                    'Place of supply' => $order->seller->shop->state,
                    'Place of delivery' => $order->seller->shop->state,
                    'Shipping to' =>  ($order->shippingAddress->contact_person_name ?? '') . ', ' . 
                                        ($order->shippingAddress->phone ?? '') . ', ' . 
                                        ($order->shippingAddress->address ?? '') . ', ' . 
                                        ($order->shippingAddress->city ?? '') . ', ' . 
                                        ($order->shippingAddress->zip ?? ''),
                    'Billing address' =>  ($order->billingAddress['contact_person_name'] ?? '') . ', ' . 
                    ...
                    'Wallet deduction' => BackEndHelper::set_symbol(BackEndHelper::usd_to_currency($tran->order->wallet_deduction)),
                    ...
                    'Status' => $tran->status,
                    'Order Date' => date('d-m-Y h:i:s a',strtotime($order['created_at'])),
                );
                }
            }
        }
    ...
    public function Order_Sale_Report(Request $request)
    {        
        $seller_id = $request->seller_id ?? 'all';
        $product_id = $request->product_id ?? 'all';
        $selected_status = $request->delivery_status ?? 'all';
        $date_type = $request->date_type ?? '';
        $from = $request->from;
        $to = $request->to;
        $search = $request->search;
        $query_param = ['selected_status'=>$selected_status,'seller_id'=>$seller_id, 'search' => $request->search, 'date_type' => $date_type, 'from' => $from, 'to' => $to, 'product_id' => $product_id];
        $sellers = Seller::where(['status'=>'approved'])->get();

        $deliveryStatuses = DB::table('shiprocket_couriers')->get();
        
        $products = Product::active()->temporary()->where('request_status',1)->where('status',1)->get();

        $chart_data = self::order_report_chart_filter($request);

        $orders = self::all_order_table_data_filter($request)->whereNull('order_return_id')
            ->when($selected_status != 'all', function ($query) use ($selected_status) {
                $query->whereIn('id', function ($q) use ($selected_status) {
                    $q->select('order_id')
        ...
        if($orderss){
            ...
            $orders = $orders->where('seller_id',auth('seller')->id())->orderBy('id','desc')->paginate(Helpers::pagination_limit())->appends($query_param);
        ...
        return view('seller-views.transaction.order_sale', compact('deliveryStatuses','selected_status','orders', 'order_count', 'payment_data', 'chart_data', 'due_amount', 'settled_amount', 'sellers', 'seller_id', 'search', 'date_type', 'from', 'to','products','product_id','net_base_amount','net_gst','net_total_amount'));
    }
    ...
    {
        $payment_record = DB::table('made_payement') // table name as per your schema
            ->join('shops', 'made_payement.seller_id', '=', 'shops.seller_id')
            ->where('made_payement.seller_id', auth('seller')->id()) // fixed here
            ->select('made_payement.*', 'shops.name')
            ->get();

        //dd($payment_record); 

        return view('seller-views.transaction.receipt', compact('payment_record'));
    }
    ...
    {        
      $seller_id = $request->seller_id ?? 'all';
       $product_id = $request->product_id ?? 'all';
        ...
        $orders = self::all_order_table_data_filter($request)->whereNotNull('order_return_id');
        ...
        return view('seller-views.report.sale_return', compact('orders', 'order_count', 'payment_data', 'chart_data', 'due_amount', 'settled_amount', 'sellers', 'seller_id', 'search', 'date_type', 'from', 'to','products','product_id','net_base_amount','net_gst','net_total_amount'));
    }
