...
class RefundController extends Controller
{
    public function list(Request $request, $status)
    {
        ...
        if ($request->has('search')) {
           ...
           if($status === 'Filter'){
            ...
            $sku = DB::table('refund_requests')
                ->join('order_details', 'refund_requests.order_details_id', '=', 'order_details.id')
                ->join('products', 'order_details.product_id', '=', 'products.id')
                ->join('sku_product_new', function ($join) {
                    $join->on('sku_product_new.product_id', '=', 'order_details.product_id')
                        ->whereRaw("
                            TRIM(SUBSTRING_INDEX(sku_product_new.variation, '-', 1)) 
                            COLLATE utf8mb4_unicode_ci
                            = TRIM(order_details.variant) 
                            COLLATE utf8mb4_unicode_ci
                        ");
                })
                ->select(
        ...
        return view('seller-views.refund.list',compact('sku','refund_list','search'));
    }
    ...
    public function details($id)
    {
        ...
        return view('seller-views.refund.details',compact('sku','refund'));
    }
