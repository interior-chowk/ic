...
@section('content')
            ...
            <div class="col-lg-8 col-xxl-9">
                ...
                <div class="card h-100">
                    ...
                    <div class="card-body">
                                        ...
                                        @if ($detail->product)
                                            <tr>
                                                    ...
                                                    <div class="media align-items-center gap-10">
                                                        <div>
                                                            ...
                                                            <div><strong>{{ \App\CPU\translate('qty') }} :</strong>
                                                                {{ $detail->qty }}</div>
                                                            <div><strong>{{ \App\CPU\translate('HSN Code') }} :</strong>
                                                                {{ $detail->product_all_status->HSN_code }}</div>
                                                        </div>
                                                    </div>
                                                ...
                                                <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($detail['tax'])) }}
                                                    [{{ isset($detail->product_all_status->tax) ? $detail->product_all_status->tax : '' }}{{ isset($detail->product_all_status->tax_type) ? '%' : '' }}]
                                                </td>
                                                <td>{{ $orders['total_variant_mrp'] - $orders['total_listed_price'] }}
                                                </td>
                                                @php($subtotal = $detail['price'] * $detail->qty + $detail['tax'] - $detail['discount'])
                                                <td>{{ $orders['total_variant_mrp'] - ($orders['total_variant_mrp'] - $orders['total_listed_price']) }}
                                                </td>
                                            </tr>
                        ...
                        @php($wallet_deduction = $order['wallet_deduction'])
                        <div class="row justify-content-md-end mb-3">
                            <div class="col-md-9 col-lg-9">
                                <dl class="row gy-1 text-sm-right">
                                    @if ($order->instant_delivery_type == 1)
                                        <dt class="col-sm-6 font-weight-normal">{{ \App\CPU\translate('Shipping') }} +
                                            {{ \App\CPU\translate('instant_delivery_charge') }}</dt>
                                    @else
                                        <dt class="col-sm-6 font-weight-normal">
                                            {{ \App\CPU\translate('Shipping_charge') }}</dt>
                                    @endif

                                    @php($toatal_ship_inst = $shipping)
                                    @php($toatal_ship_inst_charge = 0)
                                    @if ($order->instant_delivery_type == 1)
                                    @endif
                                    ...
                                    <dd class="col-sm-6 font-weight-normal title-color">
                                        ...
                                        @if (empty($orders->shipping_cost) || $orders->shipping_cost == 0)
                                            <span style="color:#3f9339;">Free</span> <strike> ₹
                                                {{ $orders->shipping_cost_amt }}</strike>
                                        @else
                                            ₹ {{ $orders->shipping_cost }}
                                        @endif
                                    </dd>

                                    {{-- <dt class="col-5">{{ \App\CPU\translate('TOTAL BILL AMOUNT') }}</dt>
                                    <dd class="col-6 title-color">
                                        <strong>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($orders['total_variant_mrp'])) }}</strong>
                                    </dd> --}}
                                    {{-- <dt class="col-sm-6 font-weight-normal">{{ \App\CPU\translate('coupon_discount') }}
                                    </dt>
                                    <dd class="col-sm-6 font-weight-normal title-color">

                                        - ₹ {{ number_format(abs($orders->discount_amount), 2) }}
                                    </dd> --}}

                                    <dt class="col-sm-6 font-weight-normal">{{ \App\CPU\translate('wallet_deduction') }}
                                    </dt>
                                    {{-- @dd(); --}}
                                    <dd class="col-sm-6 font-weight-normal title-color">
                                        - {{ $orders->wallet_deduction }}
                                    </dd>
                                    ...
                                    <dd class="col-sm-6 title-color">
                                        {{ $orders->total_listed_price + $orders->shipping_cost + $orders->discount_amount - $orders->wallet_deduction }}
                        ...
                        <div class="row  m-5">
                            ...
                            <div class="labels-all">
                                ...
                                {{-- <div class="label-status"><b>Confirmed</b>
                                    @if ($ship->status == 'confirmed')
                                        <br><small>({{ date('d M Y H:i:s', strtotime($order->updated_at)) }})</small>
                                    @endif
                                </div>

                                <div class="label-status"><b>Processing</b>
                                    @if ($ship->status == 'processing')
                                        <br><small>({{ date('d M Y H:i:s', strtotime($order->updated_at)) }})</small>
                                    @endif
                                </div>

                                <div class="label-status"><b>Out for delivery</b>
                                    @if ($ship->status == 'out_for_delivery')
                                        <br><small>({{ date('d M Y H:i:s', strtotime($order->updated_at)) }})</small>
                                    @endif
                                </div>

                                <div class="label-status"><b>Delivered</b>
                                ...
                                <div class="label-status"><b>Confirmed</b>
                                    @if (isset($ship) && $ship->status == 'confirmed')
                                        <br><small>({{ date('d M Y H:i:s', strtotime($order->updated_at)) }})</small>
                                    @endif
                                </div>

                                <div class="label-status"><b>Processing</b>
                                    @if (isset($ship) && $ship->status == 'processing')
                                        <br><small>({{ date('d M Y H:i:s', strtotime($order->updated_at)) }})</small>
                                    @endif
                                </div>

                                <div class="label-status"><b>Out for delivery</b>
                                    @if (isset($ship) && $ship->status == 'out_for_delivery')
                                        <br><small>({{ date('d M Y H:i:s', strtotime($order->updated_at)) }})</small>
                                    @endif
                                </div>

                                <div class="label-status"><b>Delivered</b>
                                    @if (isset($ship) && $ship->status == 'delivered')
                                        <br><small>({{ date('d M Y H:i:s', strtotime($order->updated_at)) }})</small>
                                    @endif
                                </div>
                        ...
                        <div class="card-body">
                            <h4 class="mb-4 d-flex align-items-center gap-2">
                                <img src="{{ asset('/public/assets/back-end/img/seller-information.png') }}"
                                    alt="">
                                {{ \App\CPU\translate('Buyer_information') }}
                            </h4>
                            <div class="media">
                                {{-- <div class="mr-3">
                                ...
                                <div class="media-body d-flex flex-column gap-1">
                                    <span
                                        class="title-color"><strong>{{ json_decode($order->shipping_address_data)->contact_person_name }}</strong></span>
                                    ...
                                    <div class="d-flex align-items-start gap-2">
                                        <img src="{{ asset('/public/assets/back-end/img/location.png') }}"
                                            alt="">
                                        {{ json_decode($order->shipping_address_data)->address }}
                                    </div>
                                    <div>
                                        <span>{{ \App\CPU\translate('Contact') }}:</span>
                                        <strong>{{ json_decode($order->shipping_address_data)->phone }}</strong>
                                    </div>
                                    <div>
                                        <span>{{ \App\CPU\translate('City') }}:</span>
                                        <strong>{{ json_decode($order->shipping_address_data)->city }}</strong>
                                    </div>
                                    <div>
                                        <span>{{ \App\CPU\translate('zip_code') }} :</span>
                                        <strong>{{ json_decode($order->shipping_address_data)->zip }}</strong>
                                    </div>
                                    <div>
                                        <span>{{ \App\CPU\translate('State') }} :</span>
                                        <strong>{{ json_decode($order->shipping_address_data)->state }}</strong>
                                    </div>
    ...
    <script>
        ...
        function updateProgress(status) {
            ...
            var progress = document.getElementById("orderProgress");
            switch (status) {
                case "confirmed":
                    progress.value = 25;
                    break;
