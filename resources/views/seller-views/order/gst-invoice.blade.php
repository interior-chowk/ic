        ...
        <table class="bs-0 mb-1 px-10">
            <tr>
                <th class="content-position-y text-left">
                    ...
                    <p style=" margin-top: 6px; margin-bottom:0px;">{{ $order->seller->shop->billing_address }},
                ...
                <th class="content-position-y text-right">
                    ...
                    <p style=" margin-top: 6px; margin-bottom:0px;"><b>Order Date</b>
                        {{ date('d-m-Y h:i:s a', strtotime($order['created_at'])) }}</p>
                    <br>
               ...
               <p style=" margin-top: 6px; margin-bottom:0px;" ><b>Invoice Date & time</b> {{ date('d-m-Y h:i a') }}</p>-->
                    ...
                    <p style=" margin-top: 6px; margin-bottom:0px;"><b>State/UT code:
                        </b>{{ $stateId = DB::table('states')->where('name', $order->seller->shop->state)->value('id') }}
                    </p>
                    <p style=" margin-top: 6px; margin-bottom:0px;"><b>Place of supply: </b>
                        {{ $order->seller->shop->state }}</p>
                            ...
                            <tr>
                                <td>
                                    @if ($order->shippingAddress)
                                        ...
                                        <span class="h2"
                                            style="margin: 0px;">{{ \App\CPU\translate('shipping_to') }} </span>
            ...
            <table class="customers bs-0">
                    ...
                    @foreach ($order->details as $key => $details)
                        ...
                        <tr>
                            ...
                            <td>
                                ...
                                @php
                                    $skuData = isset($productData['variation'])
                                        ? json_decode($productData['variation'], true)
                                        : [];
                                @endphp
                            ...
                            <td>{{ $details->qty }}</td>
                            <td>No.</td>
                            <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($discountPrice)) }}
                            </td>
                            <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($details->qty * $amount_without_tax)) }}
                            </td>

                            <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($item_tax)) }}
                                [{{ isset($details->product_all_status->tax) ? $details->product_all_status->tax : '' }}{{ isset($details->product_all_status->tax_type) ? '%' : '' }}]
                            </td>

                            <td class="text-right">
                                {{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($details->qty * $discountPrice)) }}
                            </td>
                        </tr>
                ...
                <tbody>
                    <tr>
                        ...
                        <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($shipping_amount_rate)) }}
                        </td>
                        <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($shipping_gst)) }}
                            [18%]</td>
                        <td class="text-right">
                ...
                <tbody>
                    <tr>
                        ...
                        <td><b>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($shipping_amount_rate + $sub_total_mrp)) }}</b>
                        </td>
                        <td><b>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($total_tax)) }}</b>
                        </td>
                        <td class="text-right">
                            <b>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($total_mrp + $shipping)) }}</b>
                        </td>

                    </tr>

                    <tr>
                        <td class="text-center" style=" background-color: darkgray;" colspan="2">
                            <b>{{ \App\CPU\translate('TOTAL_BILL_AMOUNT') }}</b></td>
                        <td class="text-center" style=" background-color: darkgray;" colspan="6">
                            {{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($total_mrp + $shipping)) }}
                        </td>
                    </tr>

                    <tr>

                        <td class="text-center" colspan="2"><b>{{ \App\CPU\translate('Less:_Coupon_Discount') }}</b>
                        </td>
                    ...
                    <tr>
                        <td class="text-center" colspan="2"><b>{{ \App\CPU\translate('Less:_Wallet_Payment') }}</b>
                        </td>
                        <td class="text-center" colspan="6">
                    ...
                    <tr>
                        <td class="text-center" style=" background-color: cadetblue;" colspan="2">
                            <b>{{ \App\CPU\translate('Net_Payable_Amount') }}</b></td>
                        <td class="text-center" style=" background-color: cadetblue;" colspan="6">
                            <b>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($total_mrp + $shipping - $all_coupons - $order->wallet_deduction)) }}</b>
                        </td>
                    </tr>
            ...
            <table class="customers bs-0">
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        @if ($customer_state == $seller_state)
                            ...
                            <td colspan="2" style="border: none;">
                                {{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($tax_amount)) }}
                            </td>
                    ...
                    <tr>
                        ...
                        ?>
                        <td colspan="5" style="border-top: none;border-bottom: none;">{{ $words }} Only .
                        </td>
                    </tr>
                    ...
                    <tr>
                        ...
                        <td style="border-right: none; border-left: none;">
                            <b>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($total_tax_amount)) }}</b>
                        </td>
