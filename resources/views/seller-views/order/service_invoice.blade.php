    ...
    <div class="first">
        <table class="content-position ">
            <tr>
                ...
                <td class="text-right">
                    <img height="80" src="{{ asset("storage/app/public/company/$company_web_logo") }}"
                        alt="">
                </td>
            </tr>
        </table>
        ...
        @php
            $companyName = 'Soham Infratech';
            $companyAddress = 'Flat no. 802, Tower- A6, Habitech Panchatatva, Greater Noida West, Gautam buddha Nagar';
            $companyCity = 'Greater Noida';
            $companyCountry = 'India';
            $companyState = 'Uttar Pradesh';
            $companyPincode = '201318';
            $companyGst = '09BPPPS6876F1ZY';
        @endphp
            ...
            <tr>
                <th class="content-position-y text-left">
                    <h4 class="text-uppercase mb-1 fz-14">
                        ...
                        : {{ \App\CPU\translate($companyName) }}
                    </h4>
                    <p style=" margin-top: 6px; margin-bottom:0px;">{{ $companyAddress }}</p>,
                    {{ $companyCity }}, {{ $companyState }},
                    {{ $companyCountry }}, {{ $companyPincode }}</p>
                    <?php $seller_state = $companyState ?? null; ?>
                    @if ($order['seller_is'] != 'admin' && isset($order['seller']) && $order['seller']->gst != null)
                        <h4 class="text-capitalize fz-12">{{ \App\CPU\translate('GST') }}
                            : {{ $order['seller']->gst }}</h4>
                    @endif
                    <p style=" margin-top: 6px; margin-bottom:0px;"><b>GSTIN</b> : {{ $companyGst }}</p>
                </th>
                            ...
                            <tr>
                                <td>
                                    @if ($order->shippingAddress)
                                        ...
                                        <span class="h2" style="margin: 0px;">{{ \App\CPU\translate('Bill_to') }}
                                        </span>
                                        <div class="h4 montserrat-normal-600">
                                            @php
                                                $auth = auth('seller')->id();
                                                $shop = \App\Model\Shop::where('seller_id', $auth)->first();
                                            @endphp
                                            <p style=" margin-top: 6px; margin-bottom:0px;">
                                                {{ $shop->name ?? '' }}</p>
                                            <p style=" margin-top: 6px; margin-bottom:0px;">
                                                {{ $order['seller']->f_name }} {{ $order['seller']->l_name }}</p>
                                            </p>
                                            <p style=" margin-top: 6px; margin-bottom:0px;">
                                                {{ $order['seller']->phone ?? $shop->contact }}
                                            </p>
                                            <p style=" margin-top: 6px; margin-bottom:0px;">
                                                {{ $order['seller']->address ?? $shop->address }}
                                            </p>
                                            <p style=" margin-top: 6px; margin-bottom:0px;">
                                                {{ $order['seller']->city ?? $shop->city }}
                                                {{ $order['seller']->zip ?? $shop->pincode }}</p>
                                            <p style=" margin-top: 6px; margin-bottom:0px;"><b>GST :
                                                </b>{{ $shop->gst_no }}</p>
            ...
            <table class="customers bs-0">
                <thead>
                    <tr>
                        ...
                        <th>
                            {{ \App\CPU\translate('SGST') }}<br>9%
                        </th>
                        <th>
                            {{ \App\CPU\translate('CGST') }}<br>9%
                        </th>
                        <th>
                            {{ \App\CPU\translate('IGST') }}<br>18%
                        </th>
                        <th>
                ...
                <tbody>
                    @foreach ($order->details as $key => $details)
                        ...
                        @endif
                        @php
                            $subtotal = $discountPrice * $details->qty;

                            $item_tax = 0;
                            if (!empty($details->product_all_status->tax)) {
                                $item_tax =
                                    ($discountPrice -
                        ...
                        <tr class="no-top-border">
                            ...
                            <td>
                                <center>{{ $sku->commission_fee }}</center>
                            </td>

                            <td class="text-right">
                                ₹{{ number_format(($details->qty * $sku->listed_percent * $sku->commission_fee) / 100, 2) }}
                            </td>

                            @if ($customer_state == $seller_state)
                                <td class="text-right">
                                    {{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency((number_format($details->qty * $sku->listed_percent * $sku->commission_fee) / 100) * 0.09, 2)) }}
                                </td>
                                <td class="text-right">
                                    {{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency((number_format($details->qty * $sku->listed_percent * $sku->commission_fee) / 100) * 0.09, 2)) }}
                                </td>
                            ...
                            @else
                                ...
                                <td>-</td>
                                <td class="text-right">
                                    {{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency((number_format($details->qty * $sku->listed_percent * $sku->commission_fee) / 100) * 0.18, 2)) }}
                                </td>
                            @endif
                            <td class="text-right">
                                ₹{{ (number_format($details->qty * $sku->listed_percent * $sku->commission_fee) / 100) * 0.18 +
                ...
                <tbody>
                    <tr style="border-top: none;">
                        ...
                        <td>Platform fee
                            ...
                            <br>
                            <small><i>Order date
                                    {{ date('d-m-Y h:i:s a', strtotime($order['created_at'])) }}</i></small>
                        </td>
                        <td class="text-right">
                            {{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($sku->listed_price * $details->qty + $order->shipping_cost + $order->discount_amount)) }}
                        </td>
                        <td>
                            <center>5</center>
                        </td>
                        <td class="text-right">
                            {{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($sku->listed_percent * 0.05)) }}
                        </td>
                        @if ($customer_state == $seller_state)
                            <td class="text-right">
                                {{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($sku->listed_percent * 0.05 * 0.09)) }}
                            </td>
                            <td class="text-right">
                                {{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($sku->listed_percent * 0.05 * 0.09)) }}
                            </td>
                        ...
                        @else
                            ...
                            <td class="text-center">-</td>
                            <td class="text-right">
                                {{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($sku->listed_percent * 0.05 * 0.18)) }}
                            </td>
                        @endif

                        <td class="text-right">
                            {{ \App\CPU\BackEndHelper::set_symbol(
                                \App\CPU\BackEndHelper::usd_to_currency($sku->listed_percent * 0.05) + $sku->listed_percent * 0.05 * 0.18,
                            ) }}
                        </td>
                    </tr>
                </tbody>
                        ...
                        <td class="text-right" style=" background-color: darkgray;">
                            {{ \App\CPU\BackEndHelper::set_symbol(
                                \App\CPU\BackEndHelper::usd_to_currency(
                                    ($details->qty * $sku->listed_percent * $sku->commission_fee) / 100 + $sku->listed_percent * 0.05,
                                ),
                            ) }}
                        </td>
                        <td class="text-right" style=" background-color: darkgray;">
            ...
            <table class="customers bs-0">
                <thead>
                    <tr>
                        ...
                        <th style="background-color: #ed672f;color: #fff;" class="text-center" colspan="5">Payment
                            ref. no</th>
                    </tr>
                </thead>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        $number = (($details->qty * $sku->listed_percent * $sku->commission_fee) / 100) * 0.18 + ($details->qty * $sku->listed_percent * $sku->commission_fee) / 100 + $sku->listed_percent * 0.05 + $sku->listed_percent * 0.05 * 0.18;
                    ...
                    <tr>
                        <td style="border-right: none;" colspan="5">
                            ...
                            If you require any assistance or have feedback or suggestions about our services, You can
                            connect us at <a href="mailto:support@interiorchowk.com">support@interiorchowk.com</a> or
                            {{ $company_phone }}
                        </td>
