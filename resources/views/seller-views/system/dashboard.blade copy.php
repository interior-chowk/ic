    ...
    <div class="dashboard-container">
        ...
        <div class="card dashboard-header">
            ...
            <ul class="nav nav-pills mb-3" id="dateTabs">
                ...
                {{-- <li class="nav-item">
                    <a class="nav-link {{ $date_type == 'custom_date' ? 'active' : '' }}" href="javascript:void(0)"
                        onclick="setDateType('custom_date')">
                        {{ \App\CPU\translate('Custom') }}
                    </a>
                </li> --}}
                            ...
                            <div class="col-sm-12 col-md-12">
                                <div class="form-floating">
                                    <input type="text" name="date_range" id="date_range"
                                        class="form-control __form-control" placeholder="Select Date Range" readonly>
        ...
        <div class="dashboard-summary">
            <div class="card first-div">
                ...
                <div class="card-desc green">Order Delivered = ₹{{ $deliveredIncome }}</div>
                <div class="card-desc green">Order in Progress = ₹{{ max(0, $totalIncome - $deliveredIncome) }}</div>
            </div>
            <div class="card first-div">
                <div class="card-title">Total Earnings</div>
                <div class="card-value">
                    ₹{{ $totalearning[auth('seller')->id()] ?? $earning }}
                </div>
                <div class="card-desc green">
                    Order Delivered = ₹{{ max(0, ($totalearning[auth('seller')->id()] ?? 0) - ($earning ?? 0)) }}
                </div>
                <div class="card-desc green">
                    Order in Progress = ₹{{ $earning ?? 0 }}
                </div>
            </div>
            <div class="card first-div">
                <div class="card-title">Total Collection</div>
                <div class="card-value">
                    ₹{{ number_format($paidTotalEarning + $unpaidTotalEarning, 2) }}
                </div>
                <div class="card-desc green">Paid -{{ number_format($paidTotalEarning, 2) }}</div>
                <div class="card-desc green">Claimed / unpaid -
                    {{ number_format($unpaidTotalEarning, 2) }}</div>
            </div>
        ...
        <div class="charts-container">
            ...
            {{-- <div class="card chart-card" style="max-width:300px !important;">
                ...
                <div class="gauge-container">
                    <svg viewBox="0 0 120 60" width="250" height="160">
                        ...
                        <path d="M10,55 A50,45 0 1,1 110,55" stroke="#6B394A" stroke-width="10" fill="none"
                            stroke-dasharray="100,60" />
                    </svg>
                    <div class="gauge-label">75.55%</div>
                    <div class="gauge-percent green">+10%</div>
                </div>
                <div class="gauge-desc">You succeed earn <strong>$240</strong> today, it's higher than yesterday</div>
                <div class="gauge-footer">
                    <div><span>Target</span><br><span class="red">$20k</span></div>
                    <div><span>Revenue</span><br><span class="green">$16k</span></div>
                    <div><span>Today</span><br><span class="green">$1.5k</span></div>
                </div>
            </div> --}}
            ...
            <div class="card chart-card" style="max-width:300px !important;">
                ...
                <div id="target-section">
                    ...
                    <div id="target-form" style="display:none; margin-top:10px;">
                        <input type="number" id="target-input" placeholder="Enter target (₹)" />
                ...
                <div class="gauge-container" id="gauge-container" style="display:none;">
                    ...
                    {{-- <div class="gauge-percent green" id="gauge-percent">+0%</div> --}}
                </div>

                <div class="gauge-desc" id="gauge-desc" style="display:none;">
                    You succeed earn <strong id="desc-earning">₹0</strong> today, it's higher than yesterday
                </div>
                ...
                <div class="gauge-footer" id="gauge-footer" style="display:none;">
                    ...
                    <div><span>Today</span><br><span class="green" id="footer-today">₹0</span></div>
                </div>
            </div>

            <script>
                // Backend values from Blade
                let sellerTarget = @json($sellerTarget ?? null);
                let totalIncome = @json($totalearning[auth('seller')->id()] ?? 0);
                let todayIncome = @json($todayIncome ?? 0);
                ...
                const descEarning = document.getElementById('desc-earning');
                ...
                saveBtn.addEventListener('click', () => {
                    let inputValue = parseFloat(targetInput.value);
                    if (inputValue > 0) {
                        sellerTarget = inputValue;
                        showGauge(sellerTarget);
                ...
                function showGauge(target) {
                    ...
                    footerToday.innerText = `₹${todayIncome.toLocaleString()}`;
                    descEarning.innerText = `₹${todayIncome.toLocaleString()}`;

                    let progressPercent = Math.min((totalIncome / target) * 100, 100).toFixed(2);
                    let todayPercent = ((todayIncome / target) * 100).toFixed(2);

                    gaugeLabel.innerText = `${progressPercent}%`;
                    gaugePercent.innerText = `+${todayPercent}%`;

                    // Animate stroke based on percentage
                    let pathLength = 282;
                    let dashOffset = pathLength - (progressPercent / 100) * pathLength;
                    gaugeProgress.style.strokeDasharray = `${pathLength - dashOffset}, ${pathLength}`;
                }
            </script>
            ...
            {{-- <div class="card chart-card" style="max-width:800px !important;">
                ...
                @php
                    $maxValue = max(max($monthSales), max($monthRevenue), 1);
                    ...
                    foreach (range(1, 12) as $index => $month) {
                        ...
                        $salesY = $svgHeight - ($monthSales[$month] / $maxValue) * 80;
                        $revenueY = $svgHeight - ($monthRevenue[$month] / $maxValue) * 80;
            ...
            <div class="card chart-card" style="max-width:800px !important;">
                ...
                @php
                    ...
                    foreach (range(1, 12) as $index => $month) {
                        ...
                        // ✅ ONLY CURRENT MONTH HAS VALUE (SCREENSHOT STYLE)
                        $monthSales = $month == $currentMonth ? $salesValue : 0;
                        $monthRevenue = $month == $currentMonth ? $revenueValue : 0;

                        $salesY = $svgHeight - ($monthSales / $maxValue) * 80;
                        $revenueY = $svgHeight - ($monthRevenue / $maxValue) * 80;
                ...
                <div class="side-stats-section">
                        ...
                        <div class="linechart-container">
                            <svg viewBox="0 0 300 100" width="300" height="100">
                                <polyline fill="none" stroke="#6B394A" stroke-width="5"
                                    points="0,85 30,65 60,80 90,62 120,57 150,40 180,55 210,68 240,71 270,60 300,20" />
                            </svg>
                            <div class="x-axis-labels">
                    ...
                    {{-- <div class="best-cat-card">
                        ...
                        <ul class="category-list">
                            <li><span class="cat-name">Sneaker</span><span class="cat-bar sneaker"></span></li>
                            <li><span class="cat-name">Sweatshirt</span><span class="cat-bar sweatshirt"></span></li>
                            <li><span class="cat-name">Bag</span><span class="cat-bar bag"></span></li>
                            <li><span class="cat-name">T-Shirt</span><span class="cat-bar tshirt"></span></li>
                        </ul>
                    </div> --}}
                    <div class="best-cat-card">
                        <div class="card-title">Best Selling Products Categories</div>
        ...
        <script>
            document.addEventListener("DOMContentLoaded", function() {
                ...
                // Apply selected range and auto-submit form
