                            ...
                            <form class="__shop-apply" action="{{ route('seller.auth.seller-registeration-2') }}"
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-12">
                                     ...
                                     <div class="col-md-12">
                                        <div class="form-group">
                                            <input id="pan" type="text" name="pan" value="{{ $shop?->pan ?? old('pan') }}"
                                                class="form-control" placeholder="PAN Number" required>
                                        </div>
                                    </div>
                                    ...
                                    <div class="col-md-12">
                                        <div class="form-group">                                            
                                            <select id="bank_select" name="bank_name" class="form-control">
                                                <option value="{{ $shop?->bank_name ?? old('bank_name') }}">Select Bank</option>
                                        ...
                                        <div class="c-btn-group text-center">
                                            <a href="{{ route('seller.auth.seller-registeration', ['id' => $id]) }}"
                                                class="c-btn-2">Back</a>
                                                 <button type="button" class="c-btn-2 c-btn-border" onclick="preview_data(event)">Preview</button>
...
<script>
  function preview_data(e) {
    ...
    safeSet('modal_gst_filename', gstInput ? gstInput.value.trim() : '');
    safeSet('modal_pan_filename', panInput ? panInput.value.trim() : '');
    ...
    safeSet('modal_bank_name', bankSelect && bankSelect.selectedIndex >= 0 ? bankSelect.options[bankSelect.selectedIndex].text : '');
       ...
       console.log(imageBaseUrl);
        ...
        function setVerifyStatus($input, status) {
            ...
            if (status === 'loading') {
                ...
                $icon.html(`<img src="${imageBaseUrl}right tick.webp" class="tick">`).css('height','60px').show();
        ...
        // PAN Verification
        const $pan = $('#pan');
        const $panName = $('#pan_name');
        const $panDisplay = $('#pan_holder_name_display');

        function verifyPAN() {
            const pan = $pan.val().trim();
            if (!pan) return;

            setVerifyStatus($pan, 'loading');

            $.ajax({
                url: "{{ route('seller.shop.verifyPan') }}",
                type: "POST",
                data: {
                    pan: pan,
                    _token: "{{ csrf_token() }}"
                ...
                success: function(res) {
                    if (res.success && res.data?.status === "VALID") {
                        setVerifyStatus($pan, 'success');
                        if (res.data?.full_name) {
                            $panName.val(res.data.full_name);
                            $panDisplay.text(res.data.full_name);
                        }
                    } else {
                        setVerifyStatus($pan, 'error');
                        $panName.val('');
                        $panDisplay.text('');
                        alert(res.message || 'PAN verification failed: ' + (res.data?.status ||
                            'INVALID'));
                    }
                },
                error: function() {
                    ...
                    $panName.val('');
                    $panDisplay.text('');
                    alert('Something went wrong while verifying PAN');
                }
            });
        }

        $pan.on('blur', verifyPAN);
