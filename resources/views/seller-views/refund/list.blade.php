                ...
                <div class="">
                    <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                        <img width="20" src="{{ asset('/public/assets/back-end/img/refund-request-list.png') }}"
                            alt="">
                        {{ \App\CPU\translate('return_request_list') }}
                ...
                <div class="flex-between justify-content-between align-items-center flex-grow-1">
                    ...
                    <div class="col-12 col-md-4">
                        <select name="filter" class="form-control" onchange="handleSelectChange(this)">
                            ...
                            <option value="{{ route('seller.refund.list', ['pending']) }}"
                                {{ request()->is('seller/refund/list/pending') ? 'selected' : '' }}>
                                {{ \App\CPU\translate('pending') }}</option>
                            <option value="{{ route('seller.refund.list', ['approved']) }}"
                                {{ request()->is('seller/refund/list/approved') ? 'selected' : '' }}>
                                {{ \App\CPU\translate('approved') }}</option>
                            <option value="{{ route('seller.refund.list', ['refunded']) }}"
                                {{ request()->is('seller/refund/list/refunded') ? 'selected' : '' }}>
                                {{ \App\CPU\translate('refunded') }}</option>
                            <option value="{{ route('seller.refund.list', ['rejected']) }}"
                                {{ request()->is('seller/refund/list/rejected') ? 'selected' : '' }}>
                    ...
                    <tbody>
                        @foreach ($refund_list as $key => $refund)
                            <tr>
                                ...
                                @php
                                    $refund_amount = \App\Model\Order::where('id', $refund->order_id)->first();
                                @endphp
                                <td>
                                    {{ $refund_amount->amount - $refund_amount->shipping_cost_amt }}
                                </td>
                                <td>
