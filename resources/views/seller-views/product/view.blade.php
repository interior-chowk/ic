@extends('layouts.back-end.app-seller')

@section('title', \App\CPU\translate('Product Preview'))

@push('css_or_js')
@endpush

@section('content')

    @if (!$product)
        <div class="content container-fluid text-center">
            <h3>{{ \App\CPU\translate('Product not found') }}</h3>
            <a href="{{ url()->previous() }}" class="btn btn--primary mt-3">
                <i class="tio-back-ui"></i> {{ \App\CPU\translate('Back') }}
            </a>
        </div>
    @else
        <div class="content container-fluid"
            style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};">

            <!-- Page Header -->
            <div class="page-header pb-0 mb-0 border-0">
                <div class="d-flex align-items-center justify-content-between flex-wrap gap-10 mb-3">
                    <div>
                        <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                            <img width="20"
                                onerror="this.src='{{ asset('public/assets/back-end/img/160x160/img2.jpg') }}'"
                                src="{{ asset('public/assets/back-end/img/products.png') }}" alt="">
                            {{ $product->name }}
                        </h2>
                    </div>

                    <div class="d-flex justify-content-end flex-wrap gap-10">
                        <a href="{{ url()->previous() }}" class="btn btn--primary">
                            <i class="tio-back-ui"></i> {{ \App\CPU\translate('Back') }}
                        </a>
                    </div>
                </div>

                @if ($product->request_status == 2)
                    <div class="card mb-3 mb-lg-5 mt-2 mt-lg-3 bg-warning">
                        <div class="card-body text-center">
                            <span class="text-dark">{{ $product->denied_note }}</span>
                        </div>
                    </div>
                @endif
            </div>
            <!-- End Header -->

            <!-- Rating Card -->
            <div class="card mb-3 mb-lg-5">
                <div class="card-body">
                    <div class="row align-items-md-center gx-md-5">

                        <!-- Thumbnail Rating -->
                        <div class="col-md-auto mb-3 mb-md-0">
                            <div class="d-flex align-items-center">
                                {{-- @dd($sku); --}}
                                <img class="avatar avatar-xxl avatar-4by3 {{ Session::get('direction') === 'rtl' ? 'ml-4' : 'mr-4' }}"
                                    {{-- onerror="this.src='{{ asset('public/assets/front-end/img/image-place-holder.png') }}'" --}}
                                    {{-- src="{{ Storage::url('app/public/images/' . $sku[0]->thumbnail_image) }}" --}}
                                    src="{{ rtrim(env('CLOUDFLARE_R2_PUBLIC_URL'), '/') . ($sku[0]->thumbnail_image ?? 'default.jpg') }}"
                                    alt="Image Description">

                                <div>
                                    <h4 class="display-2 text-dark mb-0">
                                        {{ count($product->rating) > 0 ? number_format($product->rating[0]->average, 2) : 0 }}
                                    </h4>
                                    <p>
                                        of {{ $product->reviews->count() }} {{ \App\CPU\translate('reviews') }}
                                    </p>
                                </div>
                            </div>
                        </div>

                        <!-- Review breakdown -->
                        <div class="col-md">
                            <ul class="list-unstyled list-unstyled-py-2 mb-0">
                                @php($total = $product->reviews->count())

                                @foreach ([5, 4, 3, 2, 1] as $star)
                                    @php($count = \App\CPU\Helpers::rating_count($product->id, $star))

                                    <li class="d-flex align-items-center font-size-sm">
                                        <span class="{{ Session::get('direction') === 'rtl' ? 'ml-3' : 'mr-3' }}">
                                            {{ $star }} {{ \App\CPU\translate('star') }}
                                        </span>

                                        <div class="progress flex-grow-1">
                                            <div class="progress-bar" role="progressbar"
                                                style="width: {{ $total ? ($count / $total) * 100 : 0 }}%;">
                                            </div>
                                        </div>

                                        <span class="{{ Session::get('direction') === 'rtl' ? 'mr-3' : 'ml-3' }}">
                                            {{ $count }}
                                        </span>
                                    </li>
                                @endforeach
                            </ul>
                        </div>

                        <div class="col-12">
                            <hr>
                        </div>

                        <!-- SKU Table -->
                        <div class="col-sm-12 pt-2">
                            @if ($sku && count($sku) > 0)
                                <table class="table table-bordered">
                                    <thead>
                                        <tr>
                                            <th>Sr. No.</th>
                                            <th>SKU Code</th>
                                            <th>Colour</th>
                                            <th>Size</th>
                                            <th>MRP (₹)</th>
                                            <th>Selling Price (₹)</th>
                                        </tr>
                                    </thead>
                                    <tbody>

                                        @foreach ($sku as $index => $item)
                                            <tr>
                                                <td>{{ $index + 1 }}</td>
                                                <td>{{ $item->sku }}</td>
                                                <td>{{ $item->color_name }}</td>
                                                <td>{{ $item->sizes }}</td>
                                                <td>₹{{ number_format($item->variant_mrp, 2) }}</td>
                                                <td>₹{{ number_format($item->listed_price, 2) }}</td>
                                            </tr>
                                        @endforeach

                                    </tbody>
                                </table>
                            @endif
                        </div>

                    </div>
                </div>
            </div>
            <!-- End Rating Section -->

            <!-- Review Table -->
            <div class="card">

                <div class="table-responsive datatable-custom">
                    <table
                        class="table table-hover table-borderless table-thead-bordered table-nowrap table-align-middle card-table"
                        style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};">

                        <thead class="thead-light thead-50 text-capitalize">
                            <tr>
                                <th>{{ \App\CPU\translate('Reviewer') }}</th>
                                <th>{{ \App\CPU\translate('Review') }}</th>
                                <th>{{ \App\CPU\translate('Date') }}</th>
                            </tr>
                        </thead>

                        <tbody>
                            @foreach ($reviews as $review)
                                @if (isset($review->customer))
                                    <tr>

                                        <!-- Reviewer -->
                                        <td>
                                            <div class="d-flex gap-3 flex-wrap align-items-center">
                                                <div class="avatar avatar-circle">
                                                    <img class="avatar-img"
                                                        {{-- onerror="this.src='{{ asset('public/assets/front-end/img/image-place-holder.png') }}'" --}}
                                                        {{-- src="{{ asset('storage/app/public/profile/' . ($review->customer->image ?? '')) }}" --}}
                                                         src="{{ rtrim(env('CLOUDFLARE_R2_PUBLIC_URL'), '/') . ($review->customer->image ?? 'default.jpg') }}"
                                                        alt="">
                                                </div>

                                                <div>
                                                    <span class="d-block h5 mb-0">
                                                        {{ $review->customer->f_name }} {{ $review->customer->l_name }}
                                                        <i class="tio-verified text-primary" data-toggle="tooltip"
                                                            title="Verified Customer"></i>
                                                    </span>

                                                    <span class="d-block font-size-sm text-body">
                                                        {{ $review->customer->email }}
                                                    </span>
                                                </div>
                                            </div>
                                        </td>

                                        <!-- Comment -->
                                        <td>
                                            <div class="text-wrap">
                                                <div class="d-flex mb-2">
                                                    <label class="badge badge-soft-info">
                                                        <span>{{ $review->rating }} <i class="tio-star"></i></span>
                                                    </label>
                                                </div>

                                                <p>{{ $review->comment }}</p>

                                                @if (json_decode($review->attachment))
                                                    @foreach (json_decode($review->attachment) as $img)
                                                        <a class="float-left"
                                                            href="{{ asset('storage/app/public/review/' . $img) }}"
                                                            data-lightbox="mygallery">

                                                            <img class="p-2" width="60" height="60"
                                                                onerror="this.src='{{ asset('public/assets/back-end/img/160x160/img2.jpg') }}'"
                                                                src="{{ asset('storage/app/public/review/' . $img) }}"
                                                                alt="">
                                                        </a>
                                                    @endforeach
                                                @endif
                                            </div>
                                        </td>

                                        <!-- Date -->
                                        <td>
                                            {{ date('d M Y H:i:s', strtotime($review->updated_at)) }}
                                        </td>

                                    </tr>
                                @endif
                            @endforeach
                        </tbody>

                    </table>
                </div>

                @if (count($reviews) == 0)
                    <div class="text-center p-4">
                        <img class="mb-3 w-160" src="{{ asset('public/assets/back-end/svg/illustrations/sorry.svg') }}"
                            alt="">
                        <p class="mb-0">{{ \App\CPU\translate('No data to show') }}</p>
                    </div>
                @endif

                <div class="table-responsive mt-4">
                    <div class="px-4 d-flex justify-content-lg-end">
                        {!! $reviews->links() !!}
                    </div>
                </div>
            </div>
            <!-- End Review Table -->

        </div>

    @endif
@endsection


@push('script_2')
    <script src="{{ asset('public/assets/back-end/js/tags-input.min.js') }}"></script>
    <script src="{{ asset('public/assets/select2/js/select2.min.js') }}"></script>

    <script>
        $('input[name="colors_active"]').on('change', function() {
            $('#colors-selector').prop('disabled', !$(this).is(':checked'));
        });

        $(document).ready(function() {
            $('.color-var-select').select2({
                templateResult: colorCodeSelect,
                templateSelection: colorCodeSelect,
                escapeMarkup: function(m) {
                    return m;
                }
            });

            function colorCodeSelect(state) {
                var colorCode = $(state.element).val();
                if (!colorCode) return state.text;
                return "<span class='color-preview' style='background-color:" + colorCode + ";'></span> " + state
                    .text;
            }
        });
    </script>
@endpush
