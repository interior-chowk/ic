                        ...
                        @foreach ($orders as $key => $order)
                            ...
                            @foreach ($orderDetails->details as $key => $details)
                                ...
                                @php
                                    $item_tax =
                                        ($details['discount'] -
                                            ($details['discount'] * 100) / ($details['tax'] + 100)) *
                                        $details['qty'];
                                    // ($discountPrice -
                                    //     ($discountPrice * 100) / ($details->product_all_status->tax + 100)) *
                                    // $details->qty;

                                    $amount_without_tax = ($details['discount'] * 100) / ($details['tax'] + 100);
                                    // ($discountPrice * 100) / ($details->product_all_status->tax + 100);
                                @endphp
                                {{-- @dd($item_tax); --}}
                                @php
                                    ...
                                    $customer_state = $billingAddress->state ?? $shippingAddress->state;
                                    $price = $details['price'] - $details['discount'];
                                    $seller = DB::table('warehouse')
                                        ->where('id', $productData['add_warehouse'])
                                        ->first();
                                ...
                                <tr>
                                    ...
                                    <td>{{ $billingAddress->contact_person_name ?? '' }}
                                    </td>
                                    <td>{{ $shippingAddress->contact_person_name ?? '' }}</td>
                                    <td>{{ $seller_state }}</td>
                                    ...
                                    <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($details['discount'])) }}
                                    </td>
                                    <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($price)) }}
                                    </td>
                                    <td>
                                        {{ $details['tax'] }}
                                    </td>
                                    <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($price - ($price * $details['tax']) / 100)) }}
                                    </td>
                                    @if ($customer_state == $seller_state)
                                        <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($item_tax / 2)) }}
                                        </td>
                                        <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($item_tax / 2)) }}
                                        </td>
                                        <td>-</td>
                                    ...
                                    <td class="text-center">
                                        {{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($price)) }}
                                    </td>
                                    @php
                                    ...
                                    <td></td>
                                    <td></td>
                                    <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(($details['qty'] * $details['discount'] * 2.1) / 100)) }}
                                    </td>
                                    <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($details['qty'] * $details['discount']) - ($details['qty'] * $details['discount'] * 2.1) / 100) }}
                                    </td>
                                ...
                                @php
                                    $countRow = $countRow + 1;
                                    $tax_amount_item = $details['tax'] / 2;
                                    $all_coupons += $order->discount_amount;
                                    $total_mrp += $details['qty'] * $details['discount'];
