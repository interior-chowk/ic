                                ...
                                <form action="{{ url()->current() }}" method="GET">
                                    <div class="input-group input-group-custom input-group-merge">
                                        ...
                                        <input id="datatableSearch_" type="search" name="search" class="form-control"
                                            placeholder="{{ \App\CPU\translate('Search_Method_Name') }}"
                                        ...
                                        <td>
                                            @foreach ($withdrawal_method['method_fields'] as $key => $method_field)
                                                <span class="badge badge-success opacity-75 fz-12 border border-white">
                                                    <b>{{ \App\CPU\translate('Name') }}:</b>
                                                    {{ \App\CPU\translate($method_field['input_name']) }} |
                                                    <b>{{ \App\CPU\translate('Type') }}:</b>
                                                    {{ $method_field['input_type'] }} |
                                                    <b>{{ \App\CPU\translate('Placeholder') }}:</b>
                                                    {{ $method_field['placeholder'] }} |
                                                    <b>{{ \App\CPU\translate('Is Required') }}:</b>
                                                    {{ $method_field['is_required'] ? \App\CPU\translate('yes') : \App\CPU\translate('no') }}
                                                </span><br />
                                            @endforeach
                                        </td>
                                        <td>
            ...
            $.ajax({
                ...
                success: function(data) {
                    if (data.success == true) {
                        toastr.success(
                            '{{ \App\CPU\translate('Default_Method_updated_successfully') }}');
                        setTimeout(function() {
                            location.reload();
                        }, 1000);
