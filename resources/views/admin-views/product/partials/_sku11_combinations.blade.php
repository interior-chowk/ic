...
<tr>
    ...
    <div id="imagePreview_{{ $key }}" class="image-preview" style="margin-top:10px; display:flex; flex-wrap:wrap;">
        @if(is_array($images))
            @foreach ($images as $img)
                <div class="image-container" draggable="true" style="position:relative; margin:5px;">
                    <input type="hidden" name="old_image_{{ $key }}[]" value="{{ $img }}">
                    <img src="{{ asset('/storage/images/'.$img) }}" width="100" alt="">
                    <input type="radio" class="image-radio" name="thumbnail_{{ $key }}" value="{{ $img }}"
                        @if($data->thumbnail_image == $img) checked @endif
                        style="position:absolute; top:5px; right:5px;">
                    <span class="remove-btn" style="position:absolute; top:5px; left:5px; cursor:pointer; font-weight:bold;">×</span>
                </div>
