...
@foreach ($combinations as $key => $combination)
    ...
    <input type="text" id="tax_gst_{{ $key }}" name="tax_gst[]"  class="form-control ms-2 text-center" style="width: 40%; height: 10%;" />
    <span class="fw-bold"> plus </span> <!-- Plus Symbol -->
        <input type="text" id="var_tax_{{ $key }}" name="var_tax[]"  class="form-control me-2 text-center" style="width: 40%; height:10%;" />
    ...
    <input type="number" placeholder="Variant MRP" name="unit_prices[]" 
           id="unit_price_{{ $key }}" value="" class="form-control unit_price mt-2">
            ...
            <td>
            <div class="d-flex align-items-center">
                <input type="text" id="selling_tax_{{ $key }}"  name="selling_taxs[]"  class="form-control me-2 text-center" style="width: 40%; height:10%;"  />
                <span class="fw-bold"> plus </span> <!-- Plus Symbol -->
                <input type="text" id="tax1_gst_{{ $key }}" name="tax1_gst[]"  class="form-control me-2 text-center" style="width: 40%; height:10%;"  />
    </div>
                <input type="text" id="selling_price_{{ $key }}" name="selling_prices[]" class="form-control selling_price mt-2"
                       value="" plcaeholder="Selling Price">
            </td>
            @if($commission->commission_fee == 3)
            <td>
                <input type="text" placeholder="{{ \App\CPU\translate('Transfer Price') }}"
                       value="" name="transfer_price[]" class="form-control" id="tp_{{ $key }}">
            </td>
            

            <td>
                <input type="text" placeholder="{{ \App\CPU\translate('Commission Fee') }}"
                       value="" name="commission_fee[]" class="form-control commission_fee" id="commission_fee_{{ $key }}" readonly>
            </td>
           @endif
            @if($commission->commission_fee == 2)
            <td>
                <input type="number" placeholder="{{ \App\CPU\translate('Commission Fee') }}"
                       value="{{$commission->fee}}" name="commission_fee[]" class="form-control commission_fee" id="commission_fee_{{ $key }}" readonly>
            </td>
            @endif
            @if($commission->commission_fee == 1)
            <td>
                <input type="number" placeholder="{{ \App\CPU\translate('Commission Fee') }}"
                       value="{{$sub_cat->commission}}" name="commission_fee[]" class="form-control commission_fee" id="commission_fee_{{ $key }}" readonly>
            </td>
            @endif
            <td>
                <input type="number" id="quant_{{ $key }}" name="quant[]" value="" class="form-control quant">
            </td>
        </tr>
    ...
    <td colspan="2" style="text-align:center;">
        ...
        <p style="background-color: {{ $color->code ?? '#ffffff' }}; width: 50px; height: 50px; margin: 0 auto; border-radius: 50%; margin-top: 10px;"></p>
        <input type="text" name="color_names[]" placeholder="write a color name" style="margin-top: 10px; text-align:center;">
    </td>
    <td colspan="5">
    <div id="dropArea_{{ $key }}" class="drop-area" style="position:relative;">
        <p style="position: relative; top: -66px;">
            &nbsp;&nbsp; Upload Images* ( Ratio 1:1 )
            <span class="ml-2" data-toggle="tooltip" data-placement="top" title="{{ \App\CPU\translate('Checked image is thumbnail image') }}">
    ...
    <input type="hidden" id="thumbnail_input_{{ $key }}" name="thumbnail_image_{{ $key }}" required>

    <div id="imagePreview_{{ $key }}" class="image-preview" name="imagePreview" style="display:flex; gap: 10px; flex-wrap: wrap;"></div>
</td>
        ...
        $.ajax({
            ...
            success: function (response) {
                ...
                response.images.forEach(function (image) {
                    imagesContainer.append(`<img src="${image.url}" alt="Uploaded Image">`);
                });
...
// document.getElementById('tax_{{$key}}').addEventListener('input', function (event) {
//     let tax = parseFloat(event.target.value); // Get the value of the input and convert to number
//     console.log('Tax: ' + tax); // Log the tax value

//     let a = (tax + 100) / 100; // Calculate the multiplier for the tax

//     // Event listener for unit price input field
//     document.getElementById('unit_price_{{$key}}').addEventListener('input', function (event) {
//         let unit = parseFloat(event.target.value); // Get the value of the unit price
//         let b = unit / a; // Calculate the price excluding tax
//         console.log('Price excluding tax: ' + b);

//         let c = (b * tax) / 100; // Calculate the GST tax
//         console.log('GST Tax: ' + c);

//         // Set values for variant tax and GST tax
//         document.getElementById('var_tax').value = b.toFixed(2); // Price excluding tax
//         document.getElementById('tax_gst').value = c.toFixed(2); // GST tax amount
...
document.querySelectorAll('[id^="imageInput_"]').forEach((input) => {
            ...
            if (file.type.startsWith('image/')) {
                ...
                reader.onload = () => {
                    ...
                    checkbox.addEventListener('change', () => {
                        const allCheckboxes = document.querySelectorAll(`#imagePreview_${key} .image-checkbox`);

                        allCheckboxes.forEach((cb) => {
                            if (cb !== checkbox) {
                                cb.checked = false;
                                cb.disabled = checkbox.checked; // Disable others
                            }
                        });
                        ...
                        if (checkbox.checked) {
                            thumbnailInput.value = file.name; // Save selected file as thumbnail
                        } else {
                    ...
                    imgContainer.addEventListener('drop', (e) => {
                        ...
                        if (draggedElement && draggedElement !== imgContainer) {
                            const draggedIndex = Array.from(previewContainer.children).indexOf(draggedElement);
                            const targetIndex = Array.from(previewContainer.children).indexOf(imgContainer);

                            if (draggedIndex < targetIndex) {
                                previewContainer.insertBefore(draggedElement, imgContainer.nextSibling);
                            } else {
                                previewContainer.insertBefore(draggedElement, imgContainer);
                            }
                            updateImageOrder(key);
                        }
                        draggedElement = null;
    ...
    function updateImageOrder(key) {
        ...
        previewContainer.querySelectorAll('input[type="hidden"][name^="uploaded_images_"]').forEach(input => {
