...
@section('title', \App\CPU\translate('Provider List'))
    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex gap-2 align-items-center">
                ...
                {{\App\CPU\translate('provider_reviews')}}
            </h2>
        </div>
        <!-- End Page Title -->
                    ...
                    <form action="{{ url()->current() }}" method="GET">
                        <div class="input-group input-group-merge input-group-custom">
                            ...
                            <input id="datatableSearch_" type="search" name="search" class="form-control"
                                placeholder="{{ \App\CPU\translate('Search by Provider or Customer') }}"
                    ...
                    <div class="col-md-4">
                        <div>
                            <label for="product" class="title-color d-flex">{{ \App\CPU\translate('choose') }}
                                {{ \App\CPU\translate('provider') }}</label>
                            <select class="form-control" name="provider_id">
                                <option value="" selected>
                                    --{{ \App\CPU\translate('select_provider') }}--</option>
                                @foreach ($providers as $provider)
                                    <option value="{{ $provider->id }}"
                                        {{ $provider_id == $provider->id ? 'selected' : '' }}>
                                        {{ Str::limit($provider->name, 20) }}</option>
                                @endforeach
                            </select>
                        </div>
                    </div>
                    ...
                    <div class="col-md-2" style="display:none;">
                        <div>
                            ...
                            <ul class="dropdown-menu dropdown-menu-right">
                                <li><a class="dropdown-item" href="{{ route('admin.reviews.export', ['provider_id' => $provider_id, 'customer_id' => $customer_id, 'status' => $status, 'from' => $from, 'to' => $to]) }}">{{ \App\CPU\translate('Excel') }}</a></li>
                ...
                <table
                    ...
                    <thead class="thead-light thead-50 text-capitalize">
                        <tr>
                            ...
                            <th>{{ \App\CPU\translate('Provider') }}</th>
                        ...
                        @foreach ($reviews as $key => $review)
                            ...
                            @if ($review->provider)
                                <tr>
                                    ...
                                    <td>
                                        <a href="{{ route('admin.provider.view', [$review['provider_id']]) }}" class="title-color hover-c1">
                                            {{ Str::limit($review->provider['name'], 25) }}
                                        </a>
                                    </td>
                                    <td>
                                        @if ($review->customer)
                                    ...
                                    <td>
                                        <label class="switcher mx-auto">
                                            <input type="checkbox"
                                                onclick="location.href='{{ route('admin.reviews.provider-review-status', [$review['id'], $review->status ? 0 : 1]) }}'"
