...
@section('content')
    <div class="content container-fluid">
        ...
        <!-- End Inlile Menu -->
        <form action="{{ route('admin.customer.customer-settings-update') }}" method="post" enctype="multipart/form-data"
            id="update-settings">
            @csrf
            <div class="row gy-2 pb-4">
                <div class="col-md-6">
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="d-flex justify-content-between align-items-center gap-10 form-control mt-4"
                        ...
                        <div class="card-body">
                            <div class="loyalty-point-section" id="customer_loyalty_point_section">
                                <div class="form-group">
                                    <label class="title-color d-flex" for="loyalty_point_exchange_rate">1
                                        {{ \App\CPU\Helpers::currency_code() }}
                                ...
                                <div class="form-group">
                                    <label class="title-color d-flex"
                                        for="intem_purchase_point">{{ \App\CPU\translate('Signup_refer_point_(sender)') }}</label>
                                    <input type="number" class="form-control" name="Signup_refer_point_sender"
                                        min="0" value="{{ $data['Signup_refer_point_sender'] ?? '0' }}">
                                </div>

                                <div class="form-group">
                                    <label class="title-color d-flex"
                                        for="intem_purchase_point">{{ \App\CPU\translate('Signup_refer_point_(receiver)') }}</label>
                                    <input type="number" class="form-control" name="Signup_refer_point_receiver"
                                        min="0" value="{{ $data['Signup_refer_point_receiver'] ?? '0' }}">
                                </div>
                            </div>
