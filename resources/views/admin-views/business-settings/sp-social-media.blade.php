    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                ...
                {{ \App\CPU\translate('service_provider_social_media') }}
            </h2>
        </div>
        <!-- End Page Title -->
                    ...
                    <div class="card-body">
                        <form style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        ...
                                        <select class="form-control w-100" name="name" id="name">
                                            ...
                                            <option value="youtube">{{ \App\CPU\translate('YouTube') }}</option>
                                        </select>
                                    </div>
                                    <div class="col-md-12 mt-2">
                                        <input type="hidden" id="id">
                                        <label for="link"
                                            class="title-color">{{ \App\CPU\translate('social_media_link') }}</label>
                            ...
                            <div class="d-flex gap-10 justify-content-end flex-wrap">
                                ...
                                <a id="update"
                                    class="btn btn--primary px-4 d--none">{{ \App\CPU\translate('update') }}</a>
                            </div>
                        </form>
                    </div>
                </div>
...
@push('script')
    <script>
        ...
        function fetch_social_media() {
            ...
            $.ajax({
                url: "{{ route('admin.business-settings.sp-fetch') }}",
                method: 'GET',
                success: function(data) {
                    ...
                    if (data.length != 0) {
                        ...
                        for (var count = 0; count < data.length; count++) {
                            ...
                            html +=
                                '<td><a type="button" class="btn btn-outline--primary btn-xs edit square-btn" id="' +
                                data[count].id + '"><i class="tio-edit"></i></a> </td></tr>';
                        }
                        $('tbody').html(html);
                    }
                }
        ...
        $('#add').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('admin.business-settings.sp-social-media-store') }}",
                method: 'POST',
                data: {
                    name: name,
                    link: link
                ...
                success: function(response) {
                    if (response.error == 1) {
                        ...
                        toastr.success(
                            '{{ \App\CPU\translate('Social Media inserted Successfully') }}.');
                    }
                    $('#name').val('');
                    $('#link').val('');
                    fetch_social_media();
        ...
        $('#update').on('click', function() {
            ...
            $.ajax({
                url: "{{ route('admin.business-settings.sp-social-media-update') }}",
                method: 'POST',
                data: {
                    id: id,
                    name: name,
        ...
        $(document).on('click', '.delete', function() {
            ...
            if (confirm("{{ \App\CPU\translate('Are you sure delete this social media') }}?")) {
                ...
                $.ajax({
                    url: "{{ route('admin.business-settings.sp-social-media-delete') }}",
                    method: 'POST',
                    data: {
                        id: id
                    },
                    success: function(data) {
                        fetch_social_media();
                        toastr.success(
                            '{{ \App\CPU\translate('Social media deleted Successfully') }}.');
                    }
                });
            }
        });
        $(document).on('click', '.edit', function() {
            ...
            $.ajax({
                url: "{{ route('admin.business-settings.sp-social-media-edit') }}",
                method: 'POST',
                data: {
                    id: id
                },
        ...
        $(document).on('change', '.status', function() {
            ...
            $.ajax({
                url: "{{ route('admin.business-settings.sp-social-media-status-update') }}",
                method: 'POST',
                data: {
                    id: id,
                    status: status
