...
@section('content')
    ...
    <!-- End Inlile Menu -->
        <form action="{{ route('admin.business-settings.shiprocket-token-update') }}" method="post"
                        ...
                        <div class="border-bottom py-3 px-4">
                            <div class="d-flex justify-content-between align-items-center gap-10">
                                <h5 class="mb-0 text-capitalize d-flex align-items-center gap-2">
                                    <i class="tio-award"></i>
                                    {{\App\CPU\translate('shiprocket_token')}}:
                                </h5>
                                <label class="switcher" for="cookie_setting_status">
                                    <input type="checkbox" class="switcher_input"
                                           ...
                                           value="1" {{isset($shiprocket_token)&&$shiprocket_token['status']==1?'checked':''}}>
                            ...
                            <div class="loyalty-point-section" id="cookie_setting_status_section">
                                <div class="form-group">
                                    <label class="title-color d-flex"
                                           for="loyalty_point_exchange_rate">{{\App\CPU\translate('shiprocket_token')}}</label>
                                    <textarea name="token" id="" cols="30" rows="6" class="form-control">{{isset($shiprocket_token) ? $shiprocket_token['value'] : ''}}</textarea>
                                </div>
