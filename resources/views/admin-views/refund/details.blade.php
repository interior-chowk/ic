...
@section('content')
    ...
    <div class="content container-fluid">
        ...
        @php($awb = App\Model\ShiprocketCourier::where('order_id', $refund->order_id)->first())
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                ...
                {{ \App\CPU\translate('return_details') }}
            </h2>
        </div>
        <!-- End Page Title -->
        ...
        <div class="row gy-2">
                ...
                <div class="card">
                    <div class="card-header">
                        <div class="row gy-1 justify-content-between align-items-center flex-grow-1">
                            <div class="col-md-4">
                                <h5 class="mb-0">{{ \App\CPU\translate('return_id') }} : {{ $refund->id }}</h5>
                            </div>
                            <h5 class="col-md-4 text-capitalize mb-0">
                                {{ \App\CPU\translate('return_status') }}:
                                ...
                                @if ($refund->status == 'pending')
                                    <span class="text--primary"> {{ \App\CPU\translate($refund->status) }}</span>
                                @elseif($refund->status == 'approved (Shipping + RTO charges will be Bear by seller)')
                                    <span class="text-success"> <abbr title="{{ $refund->status }}">Approved
                                            SB</abbr></span>
                                @elseif($refund->status == 'approved (Shipping + RTO charges will be Bear by IC/customer)')
                                    <span class="text-success"> <abbr title="{{ $refund->status }}">Approved
                                            CB</abbr></span>
                                @elseif($refund->status == 'refunded')
                                    <span class="text-info"> {{ \App\CPU\translate($refund->status) }}</span>
                            ...
                            <div class="col-md-4 d-flex justify-content-md-end">
                                <button class="btn btn--primary" data-toggle="modal"
                                    data-target="#refund-status">{{ \App\CPU\translate('change_return_status') }}</button>
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        ...
                        <div class="d-flex flex-wrap flex-column flex-md-row gap-10 justify-content-between">
                            <span class="title-color">{{ \App\CPU\translate('subtotal') }} :
                                {{ \App\CPU\Helpers::currency_converter(($refund->order_details->price - $refund->order_details->discount) * $refund->order_details->qty) }}</span>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="mb-0">{{ \App\CPU\translate('Change ID') }}</h4>
                    </div>

                    <div class="card-body">
                        <form action="{{ route('admin.refund-section.refund.order.update-id', $order->id) }}"
                            method="POST">
                            @csrf
                            @method('POST')
                            ...
                            <div class="row gy-4">
                                {{-- Row 1: Order ID --}}
                                <div class="col-md-6">
                                    <label class="form-label fw-bold">{{ \App\CPU\translate('Old Order ID') }}:</label>
                                    <div><span class="text-dark">#@if ($order->order_return_id == null)
                                                {{ $order->id }}@else{{ $order->order_return_id }}
                                            @endif
                                        </span></div>
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label">{{ \App\CPU\translate('New Order ID') }}</label>
                                    <input type="text" name="new_order_id" class="form-control"
                                        placeholder="Enter new order ID" required>
                                </div>
                                <div class="col-md-3">
                                    <label class="form-label">{{ \App\CPU\translate('Return Shipping Amount') }}</label>
                                    <input type="text" name="refund_shipping_amt" class="form-control"
                                        placeholder="Enter new shipping amt" required>
                                </div>


                                {{-- Row 2: AWB ID --}}
                                <div class="col-md-6">
                                    <label class="form-label fw-bold">{{ \App\CPU\translate('Old AWB ID') }}:</label>
                                    <div><span class="text-dark">#{{ $awb->awb_code }}</span>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                    <label class="form-label">{{ \App\CPU\translate('New AWB ID') }}</label>
                                    <input type="text" name="new_awb_id" class="form-control"
                                        placeholder="Enter new tracking ID" required>
                                </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="mb-0">{{ \App\CPU\translate('return_status_changed_log') }}</h4>
                    </div>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="mb-0">{{ \App\CPU\translate('return_reason') }}</h4>
                    </div>
                    <div class="card-body">
                        <div class="col-12">
                            <p>
            ...
            <div class="col-12">
                <div class="card">
                    <div class="card-header">
                        <h4 class="mb-0">{{ \App\CPU\translate('return_remarks') }}</h4>
                    </div>
                    <div class="card-body">
                        <div class="col-12">
                            <p>
                                {{ $refund->refund_remarks }}
                            </p>
                        </div>
                    </div>
                </div>
        ...
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h5 class="modal-title">{{ \App\CPU\translate('change_return_status') }}</h5>
                ...
                <div class="modal-body">
                    <form action="{{ route('admin.refund-section.refund.refund-status-update') }}" id='order_place'
                        ...
                        <div class="form-group col-12">
                            <label class="input-label" for="">{{ \App\CPU\translate('return_status') }}</label>
                            <select name="refund_status" class="form-control"
                                onchange="refund_status_change(this.value)">
                                <option value="approved (Shipping + RTO charges will be Bear by seller)"
                                    {{ $refund->status == 'approved (Shipping + RTO charges will be Bear by seller)' ? 'selected' : '' }}>
                                    {{ \App\CPU\translate('approved (Shipping + RTO charges will be Bear by seller)') }}
                                </option>
                                <option value="approved (Shipping + RTO charges will be Bear by IC/customer)"
                                    {{ $refund->status == 'approved (Shipping + RTO charges will be Bear by IC/customer)' ? 'selected' : '' }}>
                                    {{ \App\CPU\translate('approved (Shipping + RTO charges will be Bear by IC/customer)') }}
                                </option>
