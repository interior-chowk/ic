                                ...
                                <div class="col-sm-6 col-md-3">
                                    <select name="filter" class="form-control">
                                        ...
                                        <option value="admin" {{ $filter == 'admin' ? 'selected' : '' }}>
                                            {{ \App\CPU\translate('In_House') }}</option>
                                        <option value="seller" {{ $filter == 'seller' ? 'selected' : '' }}>
                                            {{ \App\CPU\translate('Seller') }}</option>
                                        <option value="instant_delivery"
                                            {{ $filter == 'instant_delivery' ? 'selected' : '' }}>
                                            {{ \App\CPU\translate('instant_delivery') }}</option>
                                ...
                                <div class="order-stats order-stats_failed cursor-pointer"
                                    ...
                                    <div class="order-stats__content"
                                        ...
                                        <img width="20"
                                            ...
                                            alt="">
                                        <h6 class="order-stats__subtitle">{{ \App\CPU\translate('Failed_To_Delivery') }}
                                        </h6>
                                    </div>
                                    <span class="order-stats__title">
                                        {{ $failed_count }}
                                ...
                                <form action="" method="GET">
                                    ...
                                    <div class="input-group input-group-custom input-group-merge">
                                        ...
                                        <input id="datatableSearch_" type="search" name="search" class="form-control"
                                            placeholder="{{ \App\CPU\translate('Search by Order ID or Order Status') }}"
                                            aria-label="Search by Order ID" value="{{ $search }}" required>
                                        <button type="submit"
                                    ...
                                    <div class="input-group input-group-merge input-group-custom">
                                        ...
                                        <select class="form-control" name="provider_type">
                                            <option value="" selected disabled>
                                                {{ \App\CPU\translate('provider_type') }} </option>
                                            <option value=""> {{ \App\CPU\translate('all') }} </option>
                                            <option value="2" {{ $provider_type == 2 ? 'selected' : '' }}>
                                                {{ \App\CPU\translate('Worker') }} </option>
                                            <option value="3" {{ $provider_type == 3 ? 'selected' : '' }}>
                                                {{ \App\CPU\translate('Contractor') }} </option>
                                            <option value="4" {{ $provider_type == 4 ? 'selected' : '' }}>
                                                {{ \App\CPU\translate('Architect') }} </option>
                                            <option value="5" {{ $provider_type == 5 ? 'selected' : '' }}>
                                                {{ \App\CPU\translate('Interior Designer') }} </option>
                                        </select>
                                        <button type="submit"
                                ...
                                <ul class="dropdown-menu dropdown-menu-right">
                                    <li>
                                        <a type="submit" class="dropdown-item d-flex align-items-center gap-2"
                                            href="{{ route('admin.orders.order-bulk-export', ['delivery_man_id' => request('delivery_man_id'), 'status' => $status, 'from' => $from, 'to' => $to, 'filter' => $filter, 'search' => $search]) }}">
                                            <img width="14"
                        ...
                        <table
                            ...
                            <thead class="thead-light thead-50 text-capitalize">
                                <tr>
                                    ...
                                    <th class="text-right">{{ \App\CPU\translate('Total') }}
                                        {{ \App\CPU\translate('Amount') }}</th>
                                    <th class="text-center">{{ \App\CPU\translate('Order') }}
                                ...
                                @foreach ($orders as $key => $order)
                                    <tr class="status-{{ $order['order_status'] }} class-all">
                                        ...
                                        <td>
                                            ...
                                            @php
                                                $order_detail = \App\Model\OrderDetail::where(
                                                    'order_id',
                                                    $order['id'],
                                                )->first();
                                                $product_detail = null;
                                                ...
                                                if ($order_detail && isset($order_detail['product_id'])) {
                                                    $product_detail = \App\Model\Product::where(
                                                        'id',
                                        ...
                                        <td>
                                            @if ($order->customer_id == 0)
                                                <strong
                                                    class="title-name">{{ \App\CPU\translate('walking_customer') }}</strong>
                                            @else
                                                @if ($order->customer)
                                        ...
                                        <td class="text-center text-capitalize">
                                            <div id="shipping-td-{{ $order['id'] }}">
                                                @if (isset($order['shiprocket_courier']['shiprocket_order_id']))
                                                    ...
                                                    <button type="button" title="Get Current Status"
                                                        onclick="getCurrentShippingStatus({{ $order['id'] }})"
                                                        class="btn btn-xs btn-primary"><i
                                                            class="tio-refresh"></i></button>
                                                    @if ($order['shiprocket_courier']['status'] != 'CANCELED')
                                                        <button type="button" title="Cancel Shiprocket Order"
                                                            onclick="cancelShiprocketOrder({{ $order['id'] }})"
                                                            class="btn btn-xs btn-danger"><i
                                                                class="tio-clear"></i></button>
                                                    @endif
                                                @else
                                                    <button class="btn btn-xs btn-primary"
                                                        onclick="addToShipping({{ $order['id'] }})"
                                                        type="button">Create shipping</button>
                                                @endif
                                            </div>
                                        ...
                                        <td>
                                            <div class="d-flex justify-content-center gap-2">
                                                ...
                                                <button class="btn btn-xs  <?php echo $order->pdf_label != null ? 'btn-success' : 'btn-primary'; ?>" data-toggle="modal"
                                                    data-target="#addModal" data-id="{{ $order['id'] }}">
                                                    <span class="text">{{ \App\CPU\translate('Upload') }}
                                                        {{ \App\CPU\translate('Labels') }} </span>
                                                </button>
    ...
    <div class="modal fade" tabindex="-1" role="dialog" id="addModal">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form action="{{ route('admin.orders.upload-label') }}" method="post" id="addForm"
