                        ...
                        <tr>
                            <td>
                                @if ($order->shippingAddress)
                                    ...
                                    <span class="h2" style="margin: 0px;">{{\App\CPU\translate('shipping_address')}} </span>
        ...
        <table class="customers bs-0">
            ...
            @foreach($order->details as $key=>$details)
                ...
                {{\App\CPU\translate('HSN_code')}} : {{$productData['HSN_code']}}
                      ...
                      <td><center>{{$details->qty}}</center></td>
                    <td><center>{{$productData['unit']}}</center></td>
                    <td class="text-right">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($sku->listed_percent))}}</td>
                    ...
                    <td class="text-right">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(($details->qty*($sku->listed_gst_percent))))}}  [{{ $sku->tax }}%]</td>
                   
                     

                    <td class="text-right">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(($details->qty*($sku->listed_price))))}}</td>
                </tr>
            ...
            <tbody>
                <tr>
                  ...
                  <td>-</td>  
                  <td>-</td>  
                  <td class="text-right">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($shipping_amount_rate))}}</td>    
                  <td class="text-right">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($shipping_amount_rate))}}</td>  
                 <td class="text-right">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($shipping_gst))}} [18%]</td> 
            ...
            <tbody>
                <tr>
                  ...
                  <td><center>{{ $total_qty }} </center></td>  
                  <td> </td>  
                  <td><b></b></td>
                  
                  <td class="text-right"><b>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($shipping_amount_rate + ($details->qty*$sku->listed_percent)))}}</b></td>  
                   <td class="text-right"><b>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($total_tax + ($details->qty*$sku->listed_gst_percent)))}}</b></td>
                  <td class="text-right"><b>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(($details->qty*$sku->listed_price) + $shipping))}}</b></td> 
                 
                </tr>
                
                    <tr>
                        <td class="text-right" style=" background-color: darkgray;" colspan="6" ><b>{{\App\CPU\translate('TOTAL_BILL_AMOUNT')}}</b></td>
                        <td class="text-right" style=" background-color: darkgray;" colspan="4">
                             {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(($details->qty*$sku->listed_price)+$shipping))}}</td>
                    </tr>
                
            <!-- @if($order->instant_delivery_type == 1)
                <tr>
                     <td class="text-right" colspan="6"><b>{{\App\CPU\translate('instant_delivery_amount')}}</b></td>
                    <td class="text-right" colspan="2">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($instant_delivery_amount))}}</td>
             </tr>
             @endif-->
                
                <tr>
                 
                  <td class="text-right" colspan="6" ><b>{{\App\CPU\translate('Less:_Coupon_Discount')}}</b></td>
                  <td class="text-right" colspan="4" >{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($all_coupons))}} </td>  
                
                </tr>
                
                 <tr>
                        <td class="text-right" colspan="6" ><b>{{\App\CPU\translate('Less:_Wallet_Payment')}}</b></td>
                    ...
                    <tr>
                        <td class="text-right" style=" background-color: cadetblue;" colspan="6" ><b>{{\App\CPU\translate('Net_Payable_Amount')}}</b></td>
                        <td class="text-right"  style=" background-color: cadetblue;" colspan="4">
                             <b>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(($details->qty*$sku->listed_price) + $shipping - $order->wallet_deduction + $all_coupons ))}}</b></td>
                    </tr>
            ...
            <tbody>
               ...
               // $tax_amount = $tax_amount_item*0.01 *($sub_total_mrp + $shipping_amount_rate)
                ...
                <tr>
                  ...
                  <td style="border: none;">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(($shipping_amount_rate + ($details->qty*$sku->listed_percent))))}}</td>  
                 <!-- <td  style="border: none;">{{ isset($details->product_all_status->tax)? $tax_amount_item : ''}}{{ isset($details->product_all_status->tax_type)? '%' : ''}}</td> --> 
                  <td colspan="2" style="border: none;">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(((($shipping + ($details->qty*$sku->listed_percent))*.09))))}}</td> 
                ...
                <tr>
                  ...
                  <td style="border: none;">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($shipping_amount_rate + ($details->qty*$sku->listed_percent)))}}</td>  
                 <!-- <td  style="border: none;">{{ isset($details->product_all_status->tax)? $tax_amount_item : ''}}{{ isset($details->product_all_status->tax_type)? '%' : ''}}</td> --> 
                  <td colspan="2" style="border: none;">{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency(($shipping_amount_rate + ($details->qty*$sku->listed_percent)*.18)))}}</td>  
                  <?php 
                 ...
                 <tr >
                  ...
                  <td  style="border-right: none; border-left: none;"><b>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($total_tax + ($details->qty*$sku->listed_gst_percent)))}}</b></td>
