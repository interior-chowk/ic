...
table {
    ...
    width: 100%; /* Optional: make table use full width */
}
...
th {
    background-color: #f2f2f2; /* Optional: adds background to header */
}
</style>
@section('content')
                    ...
                    <div class="card-body">
                                ...
                                @foreach($order->details as $key=>$detail)
                                    @if($detail->product_all_status)
                                        <tr>
                                            ...
                                            <td>
                                                <div class="media align-items-center gap-10">
                                                    ...
                                                    <div>
                                                        ...
                                                        <div><strong>{{\App\CPU\translate('HSN Code')}} :</strong> {{$detail->product_all_status->HSN_code}}</div>
                                                    </div>
                                                </div>
                                            ...
                                            <td>
                                                {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($detail['tax']))}} [{{ isset($detail->product_all_status->tax)? $detail->product_all_status->tax : ''}}{{ isset($detail->product_all_status->tax_type)? '%' : ''}}]
                        ...
                        <div class="row justify-content-md-end mb-3">
                            <div class="col-md-9 col-lg-9">
                                <dl class="row gy-1 text-sm-right">
                                    ...
                                    <dt class="col-5">{{\App\CPU\translate('Shipping')}} + {{\App\CPU\translate('instant_delivery_charge')}}</dt>
                                    @else
                                    <dt class="col-5">{{\App\CPU\translate('Shipping_charge')}}</dt>
                                    @endif
                       ...
                       @endif
                                    ...
                                    <dd class="col-6 title-color">
                                        <strong>{{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($toatal_ship_inst))}}</strong>
                                    </dd>
                                   ...
                                   <!-- @if($order->instant_delivery_type == 1)
                                    <dt class="col-5">{{\App\CPU\translate('instant_delivery_charge')}}</dt>
                                    <dd class="col-6 title-color">
                                        <strong> {{\App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($toatal_ship_inst_charge))}}</strong>
                                    </dd>
                                     @endif-->
                                    ...
                                    <dd class="col-6 title-color">
                                     ...
                                     <dt class="col-5">{{\App\CPU\translate('wallet_deduction')}}</dt>
                                    <dd class="col-6 title-color">
                                        - {{$wallet_deduction}}
                                    </dd>
                        ...
                        <div class="labels-all">
                            @php($shiproket_status = \App\Model\ShiprocketCourier::where('order_id',$order['id'])->first())
                         
                            @if($shiproket_status)
                             <div class="label-status"><b>Confirmed</b>
                            @if($shiproket_status['status'] =='NEW')
                             <br><small>({{date('d M Y H:i:s',strtotime($shiproket_status->updated_at))}})</small>
                             @endif
                            </div>
                           
                            <div class="label-status"><b>Processing</b>
                            @if($shiproket_status['status'] =='PICKUP SCHEDULED')
                             <br><small>({{date('d M Y H:i:s',strtotime($shiproket_status->updated_at))}})</small>
                             @endif
                            </div>
                            
                            <div class="label-status"><b>Out for delivery</b>
                            @if($shiproket_status['status'] =='Out for Delivery')
                             <br><small>({{date('d M Y H:i:s',strtotime($shiproket_status->updated_at))}})</small>
                             @endif
                            </div>
                            
                            <div class="label-status"><b>Delivered</b>
                            @if($shiproket_status['status'] =='Delivered')
                             <br><small>({{date('d M Y H:i:s',strtotime($shiproket_status->updated_at))}})</small>
                             @endif
                            </div>
                         ...
                         <div><table>
                           ...
                           <td>{{$detail->cancellation_remarks}}</td>
                            </tr>
                             @endforeach
                         </table></div>
@endif
                        ...
                        <ul class="list-unstyled list-unstyled-py-4">
                            <li>
                                ...
                                <select class="form-control text-capitalize" name="delivery_type" onchange="choose_delivery_type(this.value)">
                                    ...
                                    <option value="self_delivery" {{$order->delivery_type=='self_delivery'?'selected':''}} style="display:none;">
    ...
    <script>
       ...
       function updateProgress(status) {
         ...
         function updateProgress_ship(status) {
