...
@section('title', \App\CPU\translate('Coupon Register Report'))
    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                ...
                {{ \App\CPU\translate('Coupon_Register_Report') }}
            </h2>
        </div>
        <!-- End Page Title -->
                ...
                <table id="datatable" style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};"
                    ...
                    <thead class="thead-light thead-50 text-capitalize">
                        <tr>
                            ...
                            <th>{{ \App\CPU\translate('Admin Discount') }}</th>
                            <th>{{ \App\CPU\translate('Seller Discount') }}</th>
...
@else
    ...
    @endif
                            ...
                            <tr>
                                ...
                                <td>IC/VN{{ $order->seller->id }}/{{ $order->id }}</td>
                                <!-- <td>{{ date('d-m-Y', strtotime($order['created_at'])) }}</td> -->
                                <td>{{ $order->coupon_discount_bearer == 'inhouse' ? $order->discount_amount : '-' }}</td>
                                <td>{{ $order->coupon_discount_bearer != 'inhouse' ? '-' : '-' }}</td>
                                <!-- <td>{{ $order->billingAddress ? $order->billingAddress['contact_person_name'] : '' }}</td>
                                        <td>{{ $order->shippingAddress ? $order->shippingAddress['contact_person_name'] : '' }}
                                       <?php $customer_state = json_decode($order->shipping_address_data)->state ?? 'State not available'; ?>
