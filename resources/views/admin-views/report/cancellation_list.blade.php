...
@section('title', \App\CPU\translate('Cancellation Report'))
    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                ...
                {{ \App\CPU\translate('Cancellation_Report') }}
            </h2>
        </div>
        <!-- End Page Title -->
                    ...
                    <div class="coupon__discount w-100 text-right d-flex justify-content-between">
                        <div class="text-center">
                            ...
                            <div class="d-flex">
                                ...
                                <span class="ml-2" data-toggle="tooltip" data-placement="top"
                                    title="{{ \App\CPU\translate('this_count_is_the_summation_of') }} {{ \App\CPU\translate('failed_to_deliver') }}, {{ \App\CPU\translate('canceled') }}, {{ \App\CPU\translate('and') }} {{ \App\CPU\translate('returned_orders') }}">
                        ...
                        <div class="text-center">
                            ...
                            <div class="d-flex">
                                ...
                                <span class="ml-2" data-toggle="tooltip" data-placement="top"
                                    title="{{ \App\CPU\translate('this_count_is_the_summation_of') }} {{ \App\CPU\translate('pending') }}, {{ \App\CPU\translate('confirmed') }}, {{ \App\CPU\translate('packaging') }}, {{ \App\CPU\translate('out_for_delivery_orders') }}">
                                    <img class="info-img" src="{{ asset('/public/assets/back-end/img/info-circle.svg') }}"
                                        alt="img">
                                </span>
                            </div>
                        ...
                        <div class="text-center">
                            ...
                            <div class="d-flex">
                                ...
                                <span class="ml-2" data-toggle="tooltip" data-placement="top"
                                    title="{{ \App\CPU\translate('this_count_is_the_summation_of_delivered_orders') }}">
                                    <img class="info-img" src="{{ asset('/public/assets/back-end/img/info-circle.svg') }}"
                                        alt="img">
                                </span>
                            </div>
                    ...
                    <div class="coupon__discount w-100 text-right d-flex justify-content-between">
                        <div class="text-center">
                            ...
                            <div class="d-flex">
                                ...
                                <span class="trx-y-2 ml-2" data-toggle="tooltip" data-placement="top"
                                    title="{{ \App\CPU\translate('the_ongoing_order_amount_will_be_shown_here') }}">
                        ...
                        <div class="text-center">
                            ...
                            <div class="d-flex">
                                ...
                                <span class="trx-y-2 ml-2" data-toggle="tooltip" data-placement="top"
                                    title="{{ \App\CPU\translate('after_the_order_is_delivered_total_order_amount_will_be_shown_here') }}">
                                    <img src="{{ asset('/public/assets/back-end/img/info-circle.svg') }}" alt="img">
                                </span>
                            </div>
                        </div>
                    ...
                    <div class="card-body px-0 pt-0">
                        <div class="position-relative pie-chart">
                            ...
                            <div class="total--orders">
                                ...
                                <span>{{ \App\CPU\translate('completed') }} <br>
                                    {{ \App\CPU\translate('payments') }}</span>
                            </div>
                            <!-- Total Orders -->
                        </div>
                        <div class="apex-legends">
                            ...
                            <div class="before-bg-0177CD">
                                <span>{{ \App\CPU\translate('Digital_Payments') }}
                                    ({{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($payment_data['digital_payment'])) }})</span>
                                &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;
                            </div>
                ...
                <table id="datatable" style="text-align: {{ Session::get('direction') === 'rtl' ? 'right' : 'left' }};"
                    ...
                    <thead class="thead-light thead-50 text-capitalize">
                        <tr>
                            ...
                            <th>{{ \App\CPU\translate('Total Bill Amount') }}</th>
                            <!-- <th>{{ \App\CPU\translate('Product Discount') }}</th> -->
                            <th>{{ \App\CPU\translate('Coupon Discount') }}</th>
                            <th>{{ \App\CPU\translate('Wallet deduction') }}</th>
                            <th>{{ \App\CPU\translate('Total Receivable Amount') }}</th>
                            ...
                            <th>{{ \App\CPU\translate('Reference Code') }}</th>
                            <th class="text-center">{{ \App\CPU\translate('Payment Status') }}</th>
                    ...
                    <tbody>
                        @foreach ($orders as $key => $order)
                            <tr>
                                ...
                                <td>
                                    <a class="title-color" href="{{ route('admin.orders.details', ['id' => $order->id]) }}"
                                        target="_blank">{{ $order->id }}</a>
                                </td>
                                <td>{{ date('d-m-Y', strtotime($order['created_at'])) }}</td>
                                <td>IC/VN{{ $order->seller->id }}/{{ $order->id }}</td>
                                <td>{{ number_format($order->order_amount + $order->discount_amount - $order->shipping_cost, 2) }}
                                </td>
                                <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($order->shipping_cost)) }}
                                </td>
                                <td>{{ number_format($order->order_amount + $order->discount_amount, 2) }}</td>
                                <!-- <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($order->details_sum_discount)) }}</td> -->
                                <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($order->discount_amount)) }}
                                </td>
                                <td> -</td>
                                ...
                                <td>{{ ($order->order_amount * 2.1) / 100 }}</td>
                                <td>{{ $order->order_amount - ($order->order_amount * 2.1) / 100 }}</td>
                                <td>{{ str_replace('_', ' ', $order['transaction_ref']) }}
                                    {{ $order->payment_method == 'offline_payment' ? '(' . $order->payment_by . ')' : '' }}</td>
