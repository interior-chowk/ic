...
@section('title', \App\CPU\translate('Sales Register Report'))
    ...
    <div class="content container-fluid">
        ...
        <div class="mb-3">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                ...
                {{ \App\CPU\translate('Sales_Register_Report') }}
            </h2>
        </div>
        <!-- End Page Title -->
                        ...
                        @foreach ($orders as $key => $order)
                            ...
                            @foreach ($orderDetails->details as $key => $details)
                                ...
                                <tr>
                                    ...
                                    <td>{{ $sku_product->variant_mrp - $sku_product->listed_price }}</td>
                                    <td>{{ $sku_product->listed_price }}</td>
                                    <td>{{ $sku_product->tax }}</td>
                                    <td>{{ $sku_product->listed_percent }}</td>
                                    @if ($customer_state == $seller_state)
                                        <td>{{ $sku_product->discount_percent / 2 }}</td>
                                        <td>{{ $sku_product->discount_percent / 2 }}</td>
                                        <td>-</td>
                                    @else
                                        <td>-</td>
                                        <td>-</td>
                                        <td>{{ $sku_product->discount_percent }}</td>
                                    @endif
                                    <td>{{ $sku_product->listed_price * $details->qty }}</td>
