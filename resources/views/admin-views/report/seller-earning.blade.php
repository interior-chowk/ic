                    ...
                    <div class="card-body px-0 pt-0">
                        <div class="position-relative pie-chart">
                            ...
                            <div class="total--orders">
                                <h3>{{ \App\CPU\BackEndHelper::currency_symbol() }}{{ \App\CPU\BackEndHelper::format_currency(\App\CPU\BackEndHelper::usd_to_currency($payment_data['wallet_amount'])) }}
                                </h3>
                        ...
                        <div class="apex-legends">
                            <div class="before-bg-004188">
                                <span>{{ \App\CPU\translate('Withdrawble Balance') }}
                                    ({{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($payment_data['withdrawable_balance'])) }})</span>
                            </div>
                            <div class="before-bg-0177CD">
                                <span>{{ \App\CPU\translate('Pending Withdraws') }}
                                    ({{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($payment_data['pending_withdraw'])) }})</span>
                            </div>
                            <div class="before-bg-A2CEEE">
                                <span>{{ \App\CPU\translate('Already Withdrawn') }}
                                    ({{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($payment_data['already_withdrawn'])) }})</span>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="card-header border-0">
                <div class="w-100 d-flex flex-wrap gap-3 align-items-center">
                    <h4 class="mb-0 mr-auto">
                        ...
                        <span
                            class="badge badge-soft-dark radius-50 fz-12">{{ count($table_earning['seller_earn_table']) }}</span>
                    </h4>
                    <div>
                        ...
                        @foreach ($table_earning['seller_earn_table'] as $key => $seller_earn)
                            ...
                            <tr>
                                ...
                                <td>
                                    <div>
                                        <h6 class="mb-1">
                                            @if (isset($seller_earn['seller_id']))
                                                <a class="title-color"
                                                    href="{{ route('admin.sellers.view', ['id' => $seller_earn['seller_id']]) }}">{{ $seller_earn['name'] }}</a>
                                            @else
                                            @endif
                                        </h6>
                                    </div>
                                </td>
                                <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($total_earn_from_order)) }}
                                </td>
                                <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($shipping_earn_table)) }}
                                </td>
                                <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($commission_given_table)) }}
                                </td>
                                <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($discount_given_table)) }}
                                </td>
                                <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($total_tax_table)) }}
                                </td>
                                <td>{{ \App\CPU\BackEndHelper::set_symbol(\App\CPU\BackEndHelper::usd_to_currency($total_refund_table)) }}
                                </td>
                                <td>
