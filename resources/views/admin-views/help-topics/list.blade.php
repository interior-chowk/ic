        ...
        <div class="col-md-12">
            <div class="card">
                <div class="card-header">
                    ...
                    <a href="{{route('admin.helpTopic.category')}}" class="btn btn--primary btn-icon-split for-addFaq">
                        <i class="tio-add"></i>
                        <span class="text">{{\App\CPU\translate('Add')}} {{\App\CPU\translate('faq')}} {{\App\CPU\translate('Category')}} </span>
                    </a>
                    ...
                    <div class="table-responsive">
                        <table
                            ...
                            <tbody>
                                ...
                                @php
                                $category = App\Model\HelpTopicCategory::where('id', $help->category_id)->first();
                                $subcategory = App\Model\HelpTopicSubCategory::where('id', $help->sub_cat_id)->first();
                                @endphp
                                <tr id="data-{{$help->id}}">
                                    <td>{{$k+1}}</td>
                                    <td>{{$category['name'] ?? null}}</td>
                                    <td>{{$subcategory['sub_cat_name'] ?? null}}</td>
                                    <td>{{$help['question']}}</td>
                                    <td>{{$help['answer']}}</td>
                                    <td>{{$help['ranking']}}</td>
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('admin.helpTopic.add-new') }}" method="post" id="addForm">
                    ...
                    <div class="modal-body"
                        ...
                        <div class="form-group">
                            <select id="subcategory" name="sub_cat_id" class="form-control" disabled>
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <form action="" method="post" id="editForm"
                ...
                <div class="modal-body">
                    <div class="form-group">
                        <select id="e_category" name="category_id" class="form-control">
                            <option value="">-- Select Category --</option>
                            @foreach($categories as $category)
                            <option value="{{ $category->id }}">{{ $category->name }}</option>
                            @endforeach
                    ...
                    <div class="form-group">
                        <select id="e_subcategory" name="sub_cat_id" class="form-control">
                            <option value="">-- Select Subcategory --</option>
                        </select>
                    </div>
...
<script>
    $(document).ready(function() {
        $('#category').change(function() {
            ...
            if (categoryId) {
                $.ajax({
                    url: "{{url('admin/helpTopic/get-subcategories')}}/" + categoryId,
                    type: 'GET',
                    dataType: 'json',
                    success: function(data) {
                        $('#subcategory').empty().append('<option value="">-- Select Subcategory --</option>');
                        $.each(data, function(key, value) {
                            $('#subcategory').append('<option name="' + value.sub_cat_name + '" value="' + value.id + '">' + value.sub_cat_name + '</option>');
                        });
                        $('#subcategory').prop('disabled', false);
                    }
                });
            } else {
                $('#subcategory').empty().append('<option value="">-- Select Subcategory --</option>').prop('disabled', true);
            }
        });
    });
</script>
    ...
    $(document).on('click', '.edit', function() {
        ...
        $.ajax({
            ...
            success: function(data) {
                ...
                $("#e_status_edit").prop('checked', data.status == 1);

                // Set category
                $('#e_category').val(data.category_id).trigger('change');

                // Load subcategories dynamically
                if (data.category_id) {
                    $.ajax({
                        url: "{{url('admin/helpTopic/get-subcategories')}}/" + data.category_id,
                        type: 'GET',
                        dataType: 'json',
                        success: function(subcats) {
                            $('#e_subcategory').empty().append('<option value="">-- Select Subcategory --</option>');
                            $.each(subcats, function(key, value) {
                                $('#e_subcategory').append('<option value="' + value.id + '">' + value.sub_cat_name + '</option>');
                            });
                            $('#e_subcategory').val(data.sub_cat_id);
                        }
                    });
                }
