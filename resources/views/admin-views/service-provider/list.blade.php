    ...
    <div class="content container-fluid">
        ...
        <div class="mb-4">
            <h2 class="h1 mb-0 text-capitalize d-flex align-items-center gap-2">
                ...
                <span
                    class="badge badge-soft-dark radius-50">{{ \App\User::whereIn('role', [2, 3, 4, 5])->where('is_active', $isactive)->count() }}</span>
            </h2>
        </div>
        <!-- End Page Title -->
                        ...
                        <form action="{{ url()->current() }}" method="GET">
                            <div class="input-group input-group-merge input-group-custom">
                                ...
                                <select class="form-control" name="provider_type">
                                    <option value="" selected disabled> {{ \App\CPU\translate('provider_type') }}
                                    </option>
                        ...
                        @foreach ($customers as $key => $customer)
                            <tr>
                                @php
                                    ...
                                    $plan = '';
                                    $verified = 0;
                                    $planPurchase = App\Model\ServiceProviderPlan::where('provider_id', $customer->id)
                                        ->latest()
                                        ->first();
                                    if ($planPurchase) {
                                        $plan = App\Model\Membership::where(
                                            'id',
                                            $planPurchase->membership_id,
                                        )->first();
                                        if ($plan) {
                                ...
                                <td>
                                    <a href="{{ route('admin.customer.view', [$customer['id']]) }}"
                                        ...
                                        <img src="{{ asset('storage/app/public/service-provider/profile') }}/{{ $customer->image }}"
                                ...
                                <td class="text-center">
                                    <label class="mx-auto switcher">
                                        <input class="switcher_input" type="checkbox"
                                            onclick="featured_status('{{ $customer['id'] }}')"
                                            {{ $customer->featured == 1 ? 'checked' : '' }}>
                                ...
                                <td>
                                    <label class="switcher">
                                        <input type="checkbox" class="switcher_input status" id="{{ $customer['id'] }}"
                                ...
                                <td>
                                    ...
                                    <form action="{{ route('admin.provider.delete', [$customer['id']]) }}" method="post"
