                ...
                <div class="card-body">
                    <form action="{{ route('admin.seo.update', $data->id) }}" method="POST">
                        ...
                        <div class="form-group">
                            <label for="page">{{ \App\CPU\translate('Page Route (e.g. about-us, contact-us)') }}</label>
                            <input type="text" name="page" class="form-control" id="page" required placeholder="e.g. about-us" value="{{ old('page', $data->page) }}">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <textarea name="meta_description" class="form-control" rows="3" id="meta_description" placeholder="Enter meta description">{{ old('meta_description', $data->meta_description) }}</textarea>
                        </div>

                        <div class="form-group">
                            <label for="meta_keywords">{{ \App\CPU\translate('Meta Keywords (comma-separated)') }}</label>
                            <input type="text" name="meta_keywords" class="form-control" id="meta_keywords" placeholder="keyword1, keyword2, ..." value="{{ old('meta_keywords', $data->meta_keywords) }}">
                        </div>

                        <div class="form-group">
                            <label for="canonical">{{ \App\CPU\translate('Canonical URL (optional)') }}</label>
                            <input type="url" name="canonical" class="form-control" id="canonical" placeholder="https://example.com/page" value="{{ old('canonical', $data->canonical) }}">
                        </div>
                        ...
                        <div class="form-group">
                            ...
                            <input type="text" name="og_title" class="form-control" id="og_title" placeholder="Enter Open Graph Title" value="{{ old('og_title', $og['title'] ?? '') }}">
                        </div>

                        <div class="form-group">
                            <label for="og_description">{{ \App\CPU\translate('OG Description') }}</label>
                            <textarea name="og_description" class="form-control" rows="3" id="og_description" placeholder="Enter Open Graph Description">{{ old('og_description', $og['description'] ?? '') }}</textarea>
                        </div>

                        <div class="form-group">
                            <label for="og_image">{{ \App\CPU\translate('OG Image URL') }}</label>
                            <input type="url" name="og_image" class="form-control" id="og_image" placeholder="https://example.com/image.jpg" value="{{ old('og_image', $og['image'] ?? '') }}">
                        </div>
