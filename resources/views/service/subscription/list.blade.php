                ...
                <div class="col-md-4">
                    <div class="plan-card" data-price="{{ $membership->price }}"
                        ...
                        @if ($membership->plan_name == 'Basic')
                            <div class="free-del">Free</div>
                        @else
                            <div class="">
                                <label>
                                    <input type="radio" name="billing_{{ $membership->id }}" value="yearly" checked
                                        onchange="changePrice(this)"> Yearly
                                </label>
                                &nbsp;&nbsp;
                                <label>
                                    <input type="radio" name="billing_{{ $membership->id }}" value="quarterly"
                                        onchange="changePrice(this)"> Quarterly
                                </label>
                            </div>
                        ...
                        @if ($membership->plan_name == 'Basic')
                            <button class="select-btn mt-3" disabled>You are on Basic Plan</button>
                        @else
                            <button class="select-btn mt-3" data-plan-id="{{ $membership->id }}" data-billing="yearly"
    ...
    <script>
        function changePrice(el) {
            ...
            if (el.value === 'quarterly') {
                let qPrice = Math.round((basePrice / 12) * 3);
                amountEl.innerText = '₹' + qPrice;
                durationEl.innerText = '/quarter';
                oldPrice.style.display = 'none';

                button.dataset.billing = 'quarterly';
                button.dataset.amount = qPrice * 100;

            } else {
                let yPrice = basePrice - yearlyDiscount;
                amountEl.innerText = '₹' + yPrice;
                durationEl.innerText = '/year';
                oldPrice.style.display = 'inline';

                button.dataset.billing = 'yearly';
                button.dataset.amount = yPrice * 100;
            }
        }
    </script>
                ...
                fetch("{{ route('service.razorpay.create.order') }}", {
                    ...
                    .then(order => {
                        ...
                        let options = {
                            ...
                            currency: "INR",
                            name: "Your Company",
                            description: "Service Membership",
