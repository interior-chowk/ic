...
class WebController extends Controller
{
    ...
    public function categories_by_category($id)
    {
      ...
      public function popular_choice(){
        $pop = Category::where('status', 1)->pluck('name','slug');
...
// public function storeWishlist_1(Request $request)
// {
//     if (!auth()->check()) {
...
//     $user = auth()->user();
//     $slug = $request->input('slug'); // note: your AJAX sends 'slug'
//     // dd($slug);
//     // Fetch the product using slug
//     $product = DB::table('sku_product_new')
//         ->join('products', 'sku_product_new.product_id', '=', 'products.id')
//         ->where('sku_product_new.id', $slug)
//         ->select('sku_product_new.*', 'products.id as product_id','sku_product_new.id as ids')
//         ->first();
...
//    //dd($product->product_id,$product->ids);
...
// }
    ...
    public function storeWishlist_1(Request $request)
    {
        ...
        // Fetch the product using the slug (SKU ID)
        $product = DB::table('sku_product_new')
            ->join('products', 'sku_product_new.product_id', '=', 'products.id')
            ->where('sku_product_new.id', $slug)
            ->select('sku_product_new.*', 'products.id as product_id', 'sku_product_new.id as ids')
    ...
    public function deleteWishlist_1(Request $request)
{
    if (!auth()->check()) {
        return response()->json(['error' => 'Login required'], 401);
    }
    ...
    $user = auth()->user();
    $slug = $request->input('slug'); // receiving product slug from AJAX
    ...
    // Find and delete wishlist entry
